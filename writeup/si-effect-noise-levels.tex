...
For instance, for one model (uniform prior and periodic encoding) identifiability is more successful at a single level of \emph{low} noise (top of that figure), whereas for another model (periodic prior and uniform encoding) a single level of \emph{high} noise is more successful (mid of that figure); however, in both cases, data from four noise levels is much more successful at narrowing down the exponent closely (bottom of that figure).
At four levels of sensory noise, both the prior and the loss function are recovered reliably.}\label{fig:known-loss-unknown-prior}
\begin{comment}
...
Rows indicate the minimum noise level in the dataset; columns indicate the maximum noise level in the dataset.
Correspondingly, panels on the diagonal correspond to settings with only one noise level.
