...
If the reference stimulus is noise-free, its decoding function is the identity, so instead of recovering \(f_1^{-1}\circ f_2\), one directly recovers each decoding function \(f_i\) itself by comparing the noise-free condition against the condition with sensory noise \(\sigma_i\), for \(i=1,2\).
