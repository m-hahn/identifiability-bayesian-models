...
\begin{align*}
    ...
    \begin{align*}
\mathbb{E}[F^{-1}(m)] 
    &= \theta + \frac{\mathbb{E}[\delta]}{F'} - \frac{F''}{2F'^3}\mathbb{E}[\delta^2] + \frac{F^{-1^{(3)}}}{3!} \mathbb{E}[\delta^3] + \frac{F^{-1^{(4)}}}{4!} \mathbb{E}[\delta^4] + \frac{F^{-1^{(5)}}}{5!} \mathbb{E}[\delta^5] + \mathbb{E}[O(\delta^6)]\\
...
We work at a general stimulus \(\theta\), set \(m=F(\theta)\), define
\[
...
\]
According to the same Taylor-expansion approach as in \citet{hahn2024unifying}, but now applied to the posterior median condition
\[
...
\]
and in the even-\(p\) case the same dependence follows from Eq.~\ref{eq:d-even-p}, with affine linear dependence on \((\log \Prior)'''(\theta)\).
\end{itemize}
