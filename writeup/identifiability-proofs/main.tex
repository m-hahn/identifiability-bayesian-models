...
\label{sec:model-space-and-measure}
...
\item $\mathcal{Y}$ has the same topology as $\mathcal{X}$; without loss of generality, we assume its size is 1.
    ...
    \item $F'$ is strictly positive and finite everywhere
...
\begin{equation}
    m = F(\theta) + \epsilon
\end{equation}
where $\epsilon \sim \mathcal{N}(0,\sigma^2)$ denotes sensory noise. If $\mathcal{X}$ (and thus $\mathcal{Y}$) is a bounded interval, sensory noise is truncated to $\mathcal{Y}$. If $\mathcal{X}$ is a circle, addition is understood modulo the circle, and $\epsilon$ is instead von Mises with concentration parameter $1/\sigma^2$.
...
We discuss the situation where they are dependent (e.g., due to efficient coding) in Section~\ref{sec:simulations-efficient-coding}.


\paragraph*{Formal Definition}
We now make Assumption~\ref{ass:informal} formal, in Assumptions~\ref{ass:measure-on-a} and \ref{ass:measure-on-prod}.
As the elements of $\mathcal{F}(X)$ are constrained to be positive and integrate to 1, it is most convenient to parameterize them in terms of their logarithms.\footnote{In fact, this corresponds to the parameterization used in the implemented fitting procedure (Section~\ref{sec:tutorial}).}
...
($L(f)$, $L(f)^{(1)}$, $\dots$, $L(f)^{(5)}$) are jointly Gaussian, with non-degenerate covariance.
Formally, this means the following: For each finite collection $(x_1, r_1), \dots, (x_t, r_t) \in \mathcal{X} \times \{0, \dots, 5\}$, the vector \[(L(f)^{(r_s)}(x_s) : s=1,\dots,t) \in \mathbb{R}^t\] is a Gaussian random vector. Furthermore, whenever $(x_s,r_s) \neq (0,0)$ ($\forall s$), the covariance matrix is nondegenerate.\footnote{Note that $L(f)(0) = 0$ by definition above, to ensure $f \mapsto L(f)$ is a 1-to-1 correspondence.}
\end{assumption}
The precise definition of the Gaussian measure on $\mathcal{F}(\mathcal{X})$ is not important for our results.
...
We now discuss some concrete example constructions for such measures.
...
\begin{equation}
    f = \sum_{n=1}^\infty \left(\frac{X_n}{n^{m+1+\varepsilon}} \cos(nx) + \frac{Y_n}{n^{m+1+\varepsilon}} \sin(nx)\right)
\end{equation}
where $\varepsilon>0$ and $X_n, Y_n \sim \mathcal{N}(0,1)$.}
On an interval stimulus space $\mathcal{X} = [0,1]$, a representation using integrated Brownian motion is available.\footnote{We sample initial values $y(0), y'(0), \dots, y^{(m-1)}(0)$ from a Gaussian, and place a Brownian motion prior on $f(\theta) := y^{(m-1)}(\theta) - y^{(m-1)}(0)$, so that \(y^{(m)}\) is continuous almost surely and hence \(y\) is \(m\)-times continuously differentiable almost surely.}
...
\end{equation}
or equivalently
\begin{equation}\label{eq:bias-first-order-alt}
...
\end{equation}
or equivalently
\begin{equation}\label{eq:var-first-order-alt}
...
\begin{enumerate}
    \item $F_1 = F_2$, with identical Fisher Information $\FI(\theta) = \frac{F_1'(\theta)^2}{\sigma^2} = \frac{F_2'(\theta)^2}{\sigma^2}$, and the priors are linked as follows:
...
= & \frac{\sigma^2}{F'^2} \frac{d}{d\theta} \log (\Prior^{(2)}) +   \frac{\tilde{p_1}-\tilde{p_2}}{2} \frac{\sigma^2 F''(\theta)}{F'(\theta)^3} + \frac{\tilde{p_1}+2}{4} \frac{d}{d\theta} \frac{\sigma^2}{F'^2}  + \mathcal{O}(\sigma^4) \\
...
\begin{align*}
 \sqrt{\mathcal{J}(\theta)} = \sqrt{4\pi} \cdot \left.\frac{\operatorname{d}}{\operatorname{d}h} \mathbb{P}(\widehat{\theta}(\theta+h) \geq \widehat{\theta}(\theta))\right|_{h=0} + \mathcal{O}(\exp(-1/\sigma))
\end{align*}
...
Now taking
\begin{align*}
 \left.\frac{\operatorname{d}}{\operatorname{d}h}  \mathbb{P}(\widehat{\theta}(\theta+h) \geq \widehat{\theta}(\theta))\right|_{h=0}
= & \left.\frac{\operatorname{d}}{\operatorname{d}h}  \mathbb{P}(F(\theta+h) - F(\theta) \geq \epsilon_2-\epsilon_1)\right|_{h=0} \\
    =&  \left.\frac{\operatorname{d}}{\operatorname{d}h}  \Phi\left(\frac{F(\theta+h) - F(\theta)}{\sqrt{2\sigma^2}}\right)\right|_{h=0} \\
...
\begin{align*}
   \log \frac{P_{lik}(F(\theta_2)|x)}{P_{lik}(F(\theta_1)|x)} = \frac{(F(\theta_1)-F(x))^2 - (F(\theta_2)-F(x))^2}{2t} = \frac{1}{2t} \left(F(\theta_1)^2 - F(\theta_2)^2 + 2F(x) \cdot (F(\theta_2)-F(\theta_1))  \right)
\end{align*}
...
\begin{equation}
    \mathcal{L}(\widehat{\theta},\theta) = \log \left[p_{prior}(\widehat{\theta}) p_{lik}(F(\theta)|\widehat{\theta})\right]
\end{equation}
...
\begin{equation}
    U = \frac{1}{\int (F'(\theta))^{\frac{\tilde{q}-\tilde{p}}{2}} \cdot \Prior(\theta) d\theta }
\end{equation}
...
\begin{align*}
    \mathcal{E}(\theta) := & \frac{(bias(\theta) - \frac{\tilde{q}+2}{4} v'(\theta))}{v(\theta)} \\
    \widehat{\Prior}(\theta) \propto & \exp\left( \int_0^\theta \mathcal{E}(\theta') d\theta' \right)
\end{align*}
Since the first two moments $bias(\theta)$ (bias) and $var(\theta)$ (variance) of the response distribution $P(\widehat{\theta}|M,\theta, \sigma)$ are given at each $\theta$, and the sensory noise obeys Gaussian distribution $\mathcal{N}(0, \sigma^2)$, we have (Section~\ref{sec:background} and Lemma~\ref{lemma:expansion}):
\begin{align*}
...
Then we prove $\widehat{\Prior}(\theta) = U \cdot (F')^{\frac{\tilde{q}-\tilde{p}}{2}} \cdot \Prior(\theta) + O(t)$.
According to the definition of $\mathcal{E}(\theta), var(\theta), bias(\theta)$, we have:
\begin{align*}
\mathcal{E}(\theta) =& \frac{(bias(\theta) - \frac{\tilde{q}+2}{4} v'(\theta))}{var(\theta)} \\
= & \frac{[\mathcal{C}(\theta)t+O(t^2)] - \frac{\tilde{q}+2}{4}[\mathcal{A}'(\theta)t + O(t^2)]}{\mathcal{A}(\theta)t + O(t^2)} \\
= & \frac{\mathcal{C}(\theta)t - \frac{\tilde{q}+2}{4}\mathcal{A}'(\theta)t + O(t^2)}{\mathcal{A}(\theta)t + O(t^2)} \\
= & \frac{\mathcal{C}(\theta)-\frac{\tilde{q}+2}{4}\mathcal{A}'(\theta)}{\mathcal{A}(\theta)} + O(t)
\end{align*}
As discussed in Section~\ref{sec:background}, we have $\mathcal{C}(\theta) = \frac{(\log \Prior)'}{F'^2}-\frac{\tilde{p}+2}{2}\cdot\frac{F''}{F'^3}$. Thus, we can plug in the expression of $\mathcal{C}(\theta)$ into the definition of $\widehat{\Prior}(\theta)$ and let $\alpha$ be the constant coefficient. Then we have:
\begin{align*}
    \widehat{\Prior}(\theta) &= \alpha \cdot \exp \left({\int_0^\theta \mathcal{E}(\theta') d\theta'}\right)\\
    &= \alpha \cdot \exp\left(\int_0^\theta \frac{\mathcal{C}(\theta')-\frac{\tilde{q}+2}{4}\mathcal{A}'(\theta')}{\mathcal{A}(\theta')}d\theta' + O(t)\right)\\
...
\subsubsection{Definition and Properties of Exceptional Set $\Omega$}
...
We also include models where the first-order bias component $\mathcal{C}_M$ vanishes entirely, i.e., where the bias is zero everywhere in the low-noise regime,
 or where the first-order variance component $\mathcal{A}_M$ is constant everywhere.\footnote{This is for technical reasons, as the proof of Lemma~\ref{lemma:tilde-estimates-motor-noise} presupposes that $\mathcal{C}_M$ and the derivatiev $\mathcal{A}_M'$ are nonzero for some stimuli. In practice, as soon as prior or encoding are nonuniform, the only way for $\mathcal{C}_M$ to vanish everywhere is for $F$ and $\Prior$ to be aligned in a very specific way to ensure perfect cancellation of attraction and repulsion.}
Then:
\begin{defin}
We define $\Omega \subset \mathfrak{M}$ as:  
\begin{align*}
\Omega := & \{M : \mathcal{C}_M \equiv 0\} \cup \{M : \mathcal{A}_M' \equiv 0\} \\
& \cup  \{M_1 \in \mathfrak{M} : \exists M_2 \in \mathfrak{M} :  M_2 \neq M_1; \mathcal{A}_{M_1} \equiv \mathcal{A}_{M_2}; \mathcal{B}_{M_1} \equiv \mathcal{B}_{M_2}; \mathcal{C}_{M_1} \equiv \mathcal{C}_{M_2}; \mathcal{D}_{M_1} \equiv \mathcal{D}_{M_2}\}
\end{align*}
\end{defin}
...
\begin{proof}
  First, when $\mathcal{A}_{M_1} \equiv \mathcal{A}_{M_2}$ and $\mathcal{C}_{M_1} \equiv \mathcal{C}_{M_2}$, then the form $M_1 = \langle F',\Prior, p\rangle $, $M_2 = \langle F',C \cdot (F')^{\frac{\tilde{p'}-\tilde{p}}{2}} \Prior, p'\rangle$ follows from the expressions for $\mathcal{A}$ and $\mathcal{C}$ (Lemma~\ref{lemma:calculation-for-b}).
    It remains to prove the other direction, i.e., that when $M_2 := \langle F',C \cdot (F')^{\frac{\tilde{p}'-\tilde{p}}{2}} \Prior, p'\rangle$, then
...
Now, $\mathcal{C}_{M_1} \equiv \mathcal{C}_{M_2}$ follows from the expression (\ref{eq:c-coefficient}):
\begin{equation}
    \mathcal{C}_M(\theta) = \frac{1}{F'^2} \frac{d}{d\theta} \log \Prior(\theta) + \frac{\tilde{p}+2}{4} \frac{d}{d\theta} \frac{1}{F'(\theta)^2} 
        \end{equation}
or equivalently
    ...
    \begin{align*}
    \mathcal{B}_M(\theta) &= \frac{2g(\theta)}{F'^2(\theta)} + \frac{7F''(\theta)^2 - 2F'(\theta)F'''(\theta)}{2F'^6(\theta)}
    \end{align*}
...
First, there is only a single choice of $F' \in \mathcal{F}(\mathcal{X})$ leading to $\mathcal{A}_M' \equiv 0$ (namely, where $F'' \equiv 0$), leading to zero measure for this set of models.
Now, for the other parts of $\Omega$, we condition on $F$ and $p \in \mathcal{P}$, and show that the set of $L(\Prior)$ such that $\langle F', \Prior, p\rangle \in \Omega$ has measure zero; the result then follows from the Fubini-Tonelli theorem. 
...
where 
\begin{equation}
    M_2 := \langle F', U \cdot (F')^{\frac{\tilde{p}'-\tilde{p}}{2}} \Prior, p'\rangle
\end{equation}
...
\subsubsection{Proof of the Theorems}
...
Let $0 < \sigma_1 < \sigma_2$ be two levels of sensory noise. % ($\alpha >1$).
Assume the %fist two moments of the
...
\end{align*}
%in the limit where $\sigma_1 \rightarrow 0$.
Constants in the $\mathcal{O}(\cdot)$ expressions depend on the ratio $\frac{\sigma_2}{\sigma_1}$, and on $F$ and $\Prior$, but not on $\theta$.

\end{thm}
and Theorem 4:
\begin{thm}[Restated from Theorem 4]\label{thm:identifiability2-motor}
Theorem 3 is not affected by adding symmetric isotropic motor noise of variance $\tau^2$, and by guessed responses appearing at a rate $0 < \gamma < 1$. The constants in the $\mathcal{O}(\cdot)$ expressions then depend additionally on the ratio $\frac{\tau}{\sigma_1}$ and on $\gamma$.
\end{thm}
Our proof applies directly to the overall result covering both Theorems 3 and 4:
\begin{thm}[Restated from Theorems 3 and 4]\label{thm:restated-thms3-4}
Consider the extended model with symmetric isotropic motor noise and guessing.
There is a subset $\Omega \subset \mathfrak{M}$ of volume $0$ such that the following holds.
Let $0 < \sigma_1 < \sigma_2$ be two levels of sensory noise. Let $\tau^2$ be the variance of motor noise.
Assume the first two moments of the response distribution $\mathbb{P}(\widehat{\theta}|M,\theta, \sigma_i)$ are given at each $\theta$ and for both $\sigma_1, \sigma_2$, under a ground-truth model $M = \langle F', \Prior,p \rangle \in \mathfrak{M}$.
There is a functional $\Phi$ mapping the collection of these distributions to $\langle \widehat{F}', \widehat{\Prior},q \rangle \in \mathfrak{M}$
such that---provided $M \not\in \Omega$---we have
\begin{align*}
    \widehat{F}(\theta) &= F(\theta) + \mathcal{O}(\sigma_2^2) \\
...
Constants in the $\mathcal{O}(\cdot)$ expressions depend on $\frac{\sigma_2}{\sigma_1}$, $\frac{\tau}{\sigma_1}$, $\gamma$, and on the regularity of $F$ and $\Prior$ and their derivatives, but not on $\theta$.
\end{thm}
...
We first note that, in the limit of small sensory and motor noise, the guessing rate $\gamma$ can be identified by investigating the rates at which stimuli $\theta$ elicit far-away responses, with an error exponentially small in $1/\sigma$. 
...
Thus, for the terms entering the $\mathcal{O}(\cdot)$ expressions in Theorem~\ref{thm:restated-thms3-4}, we have:
\begin{align*}
\frac{\sigma_2}{\sigma_1} &= \sqrt{\frac{b}{a}} &
\frac{\tau}{\sigma_1} &= \frac{\rho}{\sqrt{a}}
\end{align*}
With this in mind, we will prove $\mathcal{O}(t)$ bounds that translate to the $\mathcal{O}(\sigma_2^2)$ bounds in the theorem statement.
...
\begin{equation}
    \hat{M}_{p'} := \langle F', U F'^{\frac{\tilde{p}'-\tilde{p}}{2}} \Prior, p'\rangle
\end{equation}
...
Let $p' \in \mathcal{P}$.
There is a function $\Phi_{p'}$ mapping biases and variances to a model $\langle \widehat{F}', \widehat{\Prior}, p'\rangle$, and a function $\tilde{\mathcal{D}} \in C(\mathcal{X})$ such that, given data from the model $\langle F', \Prior, p\rangle$ ($\mathcal{C}_M(\theta)$, $\mathcal{A}_M'(\theta)$ not constant zero), we have: 
\begin{enumerate}
    \item $\|\widehat{F} - F\|_{\infty,4} = O(t)$

\item $\left\|\frac{d}{d\theta}\log\widehat{\Prior}-\frac{d}{d\theta}\log\left(U\cdot (F')^{\frac{\tilde p'-\tilde p}{2}}\Prior\right)\right\|_{\infty,2} = O(t)$
...
In the absence of motor noise, the existence of estimates $\widehat{F}$, $\widehat{\Prior}$ with the error estimates claimed already follows from the proof of Theorem 2. 
    ...
    If the observed human response has Gaussian motor noise\footnote{In fact, the expressions for $var_1, var_2, bias_1, bias_2$ are identical if motor noise is general symmetric isotropic, we hence use that condition in the theorem statement. Below, we only use the facts that motor noise has mean zero and variance $\rho^2 t$.}, we have for the response $\tilde{\theta}$:
    \begin{align*}
...
The full proof approaches this challenge by first obtaining estimates of $\frac{a}{b}$, $at$, $bt$ that control for the effect of motor noise except perhaps a higher-order residual.
...
\paragraph*{Full Proof with Motor Noise}
 ...
 Since \(\mathcal{C}\) is not identically zero,  \(\int |\mathcal C(\theta)|\,d\theta\neq 0\).
Define
        \begin{align*}
        \widehat{\left(\frac{a}{b}\right)} &:= \frac{\int |bias_1(\theta)|\,d\theta}{\int |bias_2(\theta)|\,d\theta}
        = \frac{a\int|\mathcal{C}(\theta)|\,d\theta \cdot t + \mathcal{O}(t^2)}{b\int|\mathcal{C}(\theta)|\,d\theta \cdot t + \mathcal{O}(t^2)}
        = \frac{a}{b} + \mathcal{O}(t)
        \end{align*}
        Based on this, we define the estimator
        \begin{align*}
        \widehat{b\mathcal{A}t}(\theta) &:= \frac{var_2(\theta) - var_1(\theta)}{1 - \widehat{\frac{a}{b}}}
        = \frac{var_2(\theta) - var_1(\theta)}{1 - \frac{a}{b} + O(t)}
        = b\mathcal{A}(\theta)t + O(t^2)
        \end{align*}
        and from it the scalar estimator
        \begin{align*}
        \widehat{\rho^2t} &:= \frac{1}{|\mathcal{X}|}\int \left[var_2(\theta) - \widehat{b\mathcal{A}t}(\theta)\right]d\theta
        = \rho^2t + \mathcal{O}(t^2).
    \end{align*}
...
We estimate $F$ from the variability while controlling for the addition of motor noise using the estimator $\widehat{\rho^2t}$:\footnote{An essentially equivalent approach would be to deconvolve with the variance-$\widehat{\rho^2}$ motor distribution and apply Lemma~\ref{lemma:f-exact}.}
    \begin{align*}
        ...
        \end{align*}
        As $\mathcal{A}$ is bounded away from zero, we have
        \begin{align*}
...
Based on this, set
    \begin{align*}
        ...
        Further, set (where $\theta_*$ can be chosen arbitrarily; note that $\mathcal{A}$ is nowhere zero):
        \begin{align*}
        \widehat{at} &:= \frac{var_1(\theta_*) - \widehat{\rho^2t}}{\tilde{\mathcal{A}}(\theta_*)}
        = \frac{a\mathcal{A}(\theta_*)t+O(t^2)}{\mathcal{A}(\theta_*)+O(t)}
    \end{align*}
    Similarly, we set:
    \begin{equation*}
        \widehat{bt} := \frac{var_2(\theta_*) - \widehat{\rho^2t}}{\tilde{\mathcal{A}}(\theta_*)} +O(t^2) = bt + O(t^2)
    \end{equation*}
    where $\theta_*$ can be chosen arbitrarily.
    Next, we use $\frac{bias_1(\theta)}{\widehat{at}}$ to estimate $\tilde{\mathcal{C}}(\theta)= \mathcal{C}(\theta) +O(t)$:
    \begin{align*}
        \tilde{\mathcal{C}}(\theta) := \frac{bias_1(\theta)}{\widehat{at}} &= \frac{bias_1(\theta)}{\frac{var_1(\theta) - \widehat{\rho^2t}}{\tilde{\mathcal{A}}(\theta)}}
        = \frac{a\mathcal{C}(\theta)t + O(t^2)}{at + O(t^2)}
        = \mathcal{C}(\theta) + O(t)
\end{align*}
Lemma~\ref{lemma:calculation-for-b} shows that $\mathcal{C}(\theta) = \frac{\frac{d \log P_{prior}(\theta)}{d \theta}}{F'^2(\theta)} - \frac{\tilde{p}+2}{2} \cdot \frac{F''(\theta)}{F'^3(\theta)}$.
...
%}
%
We now make this estimate explicit. Set
\begin{align*}
    G(\theta) := \frac{d}{d\theta}\log\widehat{\Prior}(\theta) - \frac{d}{d\theta}\log\left((F'(\theta))^{\frac{\tilde{p}'-\tilde{p}}{2}}\Prior(\theta)\right).
\end{align*}
The preceding calculation shows \(G(\theta)=O(t)\) uniformly in \(\theta\). Further, by Section~\ref{sec:variance-two-derivatives}, differentiating once and twice preserves the order of the remainder, so
\begin{equation}
    \|G\|_{\infty,2} = O(t).
\end{equation}
Now define
\begin{align*}
    H(\theta) := \log\widehat{\Prior}(\theta) - \log\left((F'(\theta))^{\frac{\tilde{p}'-\tilde{p}}{2}}\Prior(\theta)\right).
\end{align*}
Then \(H' = G\), hence \(\|H'\|_{\infty,2}=O(t)\). Integrating from \(0\) to \(\theta\),
\begin{align*}
    H(\theta)-H(0) = \int_0^\theta G(\theta')\,d\theta' = O(t)
\end{align*}
uniformly in \(\theta\). Therefore
\begin{align*}
    H(\theta) = c_t + O(t)
\end{align*}
for some scalar \(c_t\). Equivalently,
\begin{align*}
    \widehat{\Prior}(\theta) = e^{c_t+O(t)}\cdot (F'(\theta))^{\frac{\tilde{p}'-\tilde{p}}{2}}\Prior(\theta).
\end{align*}
Since \(\widehat{\Prior}\) and \(U\cdot (F')^{\frac{\tilde{p}'-\tilde{p}}{2}}\Prior\) are both normalized to integrate to \(1\), we must have \(e^{c_t}=U\cdot e^{O(t)}\), i.e. \(c_t=\log U + O(t)\). 
It follows that
\begin{equation}
\left\|\log \widehat{\Prior}(\theta) - (\log U + \log (F')^{\frac{\tilde{p}'-\tilde{p}}{2}}  + \log \Prior(\theta))\right\|_{\infty,3} = O(t).
\end{equation}
for the scalar
\begin{equation}
    U = \frac{1}{\int (F'(\theta))^{\frac{\tilde{p}'-\tilde{p}}{2}} \cdot \Prior(\theta) d\theta }
\end{equation}
    ...
    \begin{align*}
        \mathcal{B} &= \frac{2g}{F'^2} + \frac{7F''^2-2F'F'''}{2F'^6}
        \end{align*}
        where
        \begin{align*}
...
\end{align*}
    ...
    \begin{align*}
    \tilde{\mathcal{B}} &= \frac{2\tilde{g}}{\widehat{F}'^2} + \frac{7\widehat{F}''^2 - 2\widehat{F}'\widehat{F}'''}{2\widehat{F}'^6} %\\
     &= \frac{2g + \mathcal{O}(t)}{F'^2 + \mathcal{O}(t)} + \frac{7F''^2 - 2F'F''' + \mathcal{O}(t)}{2F'^6 + \mathcal{O}(t)} %\\
     &= \frac{2g}{F'^2} + \frac{7F''^2-2F'F'''}{2F'^6} + \mathcal{O}(t)  %\\
...
Rearranging,
    ...
    \end{align*}
    We now refine the estimate of \(\frac{a}{b}\). For models outside $\Omega$, \(\frac{d \mathcal A(\theta)}{d\theta}\) is not identically zero; choose an open interval \(I_A\subseteq\mathcal X\) on which \(\frac{d \mathcal A(\theta)}{d\theta}\) has constant sign and hence \(\int_{I_A}\frac{d \mathcal A(\theta)}{d\theta}\,d\theta\neq 0\).
    Further, define
    \begin{align*}
    ...
    \mathfrak{A} &:= \int_{I_A}\left(\frac{d var_1(\theta)}{d \theta} - \widehat{a^2t^2} \tilde{\frac{d \mathcal{B}}{d \theta}}(\theta)\right)\,d\theta
    = a\int_{I_A}\frac{d \mathcal{A}(\theta)}{d \theta}\,d\theta \cdot t + O(t^3) \\
    \mathfrak{B} &:= \int_{I_A}\left(\frac{d var_2(\theta)}{d \theta} - \widehat{b^2t^2} \tilde{\frac{d \mathcal{B}}{d \theta}}(\theta)\right)\,d\theta
    = b\int_{I_A}\frac{d \mathcal{A}(\theta)}{d \theta}\,d\theta \cdot t + O(t^3)\\
    \end{align*}    
    This now enables us to derive a precise estimate of $\frac{a}{b}$:
    \begin{align*}
    \overline{(\frac{a}{b})} &= \frac{\mathfrak{A}}{\mathfrak{B}}
    = \frac{a\int_{I_A}\frac{d \mathcal{A}(\theta)}{d \theta}\,d\theta \cdot t + O(t^3)}{b\int_{I_A}\frac{d \mathcal{A}(\theta)}{d \theta}\,d\theta \cdot t + O(t^3)}
    = \frac{a}{b}+O(t^2)
\end{align*}
...
\begin{align*}
    \mathcal{A}_M(\theta) &= \frac{1}{F'(\theta)^2} \\
    \mathcal{B}_M(\theta) &= \frac{2g(\theta)}{F'^2(\theta)} + \frac{7F''^2(\theta) - 2F'(\theta)F'''(\theta)}{2F'^6(\theta)}
\end{align*}
        where
        \begin{align*}
        g &= \frac{d}{d\theta} \mathcal{C}_{dec,M} = \frac{[(F')^2 \cdot \frac{d^2}{d \theta^2}\log \Prior - 2F'F'' \cdot \frac{d}{d \theta}\log \Prior]}{F'^4} - \frac{(\tilde{p}+1)(F'F''' - 3F''^2)}{2F'^4}
\end{align*}
...
\begin{align*}
    var(F^{-1}(m)) &= var\left(\theta + \frac{1}{F'} \cdot \delta - \frac{F''}{2F'^3} \cdot \delta^2 + \frac{3F''^2 - F'F'''}{6F'^5} \cdot \delta^3 + O(\delta^4)\right)\\
    &= var\left(\frac{1}{F'} \cdot \delta\right) + var\left(-\frac{F''}{2F'^3} \cdot \delta^2\right) + var\left(\frac{3F''^2 - F'F'''}{6F'^5} \cdot \delta^3\right)\\
    &\quad + 2cov\left(\frac{1}{F'} \cdot \delta, -\frac{F''}{2F'^3} \cdot \delta^2\right) \\
    &\quad + 2cov\left(\frac{1}{F'} \cdot \delta, \frac{3F''^2 - F'F'''}{6F'^5} \cdot \delta^3\right) \\
    &\quad + 2cov\left(-\frac{F''}{2F'^3} \cdot \delta^2, \frac{3F''^2 - F'F'''}{6F'^5} \cdot \delta^3\right) + O(\sigma^6)
\end{align*}
since 
\begin{itemize}
    \item $\mathbb{E}(\delta) = \mathbb{E}(\delta^3) = \mathbb{E}(\delta^5) = 0$
    \item $\mathbb{E}(\delta^2) = \sigma^2$
    \item $\mathbb{E}(\delta^4) = 3\sigma^4$
    \item $\mathbb{E}(\delta^6) = 15\sigma^6$
\end{itemize}
Thus, we have:
\begin{align*}
    var(\delta) &= \mathbb{E}(\delta^2) - [\mathbb{E}(\delta)]^2 = \sigma^2 \\
    var(\delta^2) &= \mathbb{E}(\delta^4) - [\mathbb{E}(\delta^2)]^2 = 3\sigma^4 - \sigma^4 = 2\sigma^4 \\
    var(\delta^3) &= \mathbb{E}(\delta^6) - [\mathbb{E}(\delta^3)]^2 = 15\sigma^6 = O(\sigma^6)\\
    cov(\delta, \delta^2) &= \mathbb{E}(\delta^3) - \mathbb{E}(\delta)\mathbb{E}(\delta^2) = 0 \\
    cov(\delta, \delta^3) &= \mathbb{E}(\delta^4) - \mathbb{E}(\delta)\mathbb{E}(\delta^3) = 3\sigma^4 - 0 = 3\sigma^4 \\
    cov(\delta^2, \delta^3) &= \mathbb{E}(\delta^5) - \mathbb{E}(\delta^2)\mathbb{E}(\delta^3) = 0
\end{align*}
In summary, substituting these back into the full expansion eliminates the zero-covariances and absorbs the higher-order variances into the $O(\sigma^6)$ term:
\begin{align*}
    var(F^{-1}(m)) &= \frac{1}{F'^2} var(\delta) + \frac{F''^2}{4F'^6} var(\delta^2) + 0 - 0 + \frac{3F''^2 - F'F'''}{3F'^6} cov(\delta, \delta^3) - 0 + O(\sigma^6)\\
    &= \frac{\sigma^2}{F'^2} + \frac{F''^2}{4F'^6} (2\sigma^4) + \frac{3F''^2 - F'F'''}{3F'^6} (3\sigma^4) + O(\sigma^6)\\
    &= \frac{\sigma^2}{F'^2} + \frac{F''^2}{2F'^6} \sigma^4 + \frac{3F''^2 - F'F'''}{F'^6} \sigma^4 + O(\sigma^6)\\
    &= \frac{\sigma^2}{F'^2} + \left( \frac{F''^2 + 6F''^2 - 2F'F'''}{2F'^6} \right) \sigma^4 + O(\sigma^6)\\
    &= \frac{\sigma^2}{F'^2} + \frac{7F''^2 - 2F'F'''}{2F'^6} \cdot \sigma^4 + O(\sigma^6)
\end{align*}
...
\begin{equation}\label{eq:derivation-of-g}
    g(\theta) = \frac{d}{d\theta} \mathcal{C}_{dec}(\theta) = \frac{[(F')^2 \cdot \frac{d^2 \log P_{prior}}{d \theta^2} - 2F'F'' \cdot \frac{d \log P_{prior}}{d \theta}]}{F'^4} - \frac{(\tilde{p}+1)(F'F''' - 3F''^2)}{2F'^4}
\end{equation}
...
\begin{align*}
var(\widehat{\theta}) 
    =& \left(\underbrace{\frac{\partial f}{\partial \theta}}_{1 + \sigma^2\mathcal{C}' + O(\sigma^4)}\right)^2 \cdot \underbrace{var(F^{-1}(m))}_{\frac{\sigma^2}{F'^2}+\frac{7F''^2-2F'F'''}{2F'^6}\cdot\sigma^4 + O(\sigma^6)} + \underbrace{\left(\frac{1}{2} \cdot \frac{\partial^2 f}{\partial \theta^2}\right)^2 \cdot var\left([F^{-1}(m)]^2\right)}_{O(\sigma^6)}\\
    ...
    =& \left(1 + \sigma^2\mathcal{C}'\right)^2 \cdot \left(\frac{\sigma^2}{F'^2}+\frac{7F''^2-2F'F'''}{2F'^6}\cdot\sigma^4\right) + O(\sigma^6) \\
    =& \sigma^2 \underbrace{\frac{1}{F'^2}}_{ \mathcal{A}(\theta)} + \underbrace{\left(\frac{2g(\theta)}{F'^2} + \frac{7F''^2-2F'F'''}{2F'^6}\right)}_{\mathcal{B}(\theta)} \cdot \sigma^4 + O(\sigma^6)
\end{align*}
...
and the coefficient $\mathcal{D}_{dec,M}$ has the following properties:
    \begin{enumerate}
        ...
        \begin{align*}
            (\log \Prior)'(\theta), \quad (\log \Prior)''(\theta), \quad (\log \Prior)'''(\theta), \\
\quad F'(\theta), \quad F''(\theta), \quad F'''(\theta), \quad F''''(\theta), \quad \frac{1}{F'(\theta)}, \quad \frac{1}{\Prior(\theta)}.
        \end{align*}
        
        \item At $p=0$, the expression $\mathcal{D}_{\langle F', \Prior, p\rangle}(\theta)$ is a polynomial in
        \[
            (\log \Prior)'(\theta), \quad (\log \Prior)''(\theta), \quad (\log \Prior)'''(\theta), \quad F'(\theta), \quad F''(\theta), \quad F'''(\theta), \quad F''''(\theta), \quad \frac{1}{F'(\theta)}
        \]
...
\begin{align*}
    F^{-1}(m) &= \theta + \frac{\delta}{F'} - \frac{F''}{2F'^3} \cdot \delta^2 + \frac{F^{-1^{(3)}}}{3!} \cdot \delta^3 + \frac{F^{-1^{(4)}}}{4!} \cdot \delta^4 + \frac{F^{-1^{(5)}}}{5!}\delta^5 + O(\delta^6).
    \end{align*}
    Since \(\delta\sim N(0,t)\), we have
    \[
    \mathbb E[\delta]=0,\qquad \mathbb E[\delta^2]=t,\qquad \mathbb E[\delta^3]=0,\qquad \mathbb E[\delta^4]=3t^2,\qquad \mathbb E[\delta^5]=0,\qquad \mathbb E[\delta^6]=15t^3.
    \]
    Therefore
    \[
    \mathbb E\!\left[\frac{F^{-1^{(5)}}}{5!}\delta^5\right]=0,
    \qquad
    \mathbb E[O(\delta^6)]=O(t^3),
    \]
    and hence
    \begin{align*}
    \mathbb E[F^{-1}(m)]
    &= \theta - \frac{F''}{2F'^3}t + \frac{F^{-1^{(4)}}}{4!}\,3t^2 + O(t^3).
    \end{align*}
    Further, we find
    \begin{align*}
...
\end{align*}
Using the expansion (and suppressing the $M$ subscript where unambiguous), using $\vartheta$ for a general stimulus distinct from $\theta$,
\begin{align*}
    f(\vartheta) = &\vartheta + t \mathcal{C}_{dec}(\vartheta) + t^2 \mathcal{D}_{dec}(\vartheta) + O(t^3) \\
...
Then
\[
\mathbb E[u]=t\mathcal C_{enc}(\theta)+t^2\mathcal D_{enc}(\theta)+O(t^3),\qquad \mathbb E[u^2]=t\mathcal A(\theta)+O(t^2),\qquad \mathbb E[u^3]=O(t^2).
\]
Evaluating the decoder expansion at \(\vartheta=\theta+u\) and Taylor-expanding in \(u\),
\begin{align*}
    f(\theta+u)
    =& \theta+u+t\mathcal C_{dec}(\theta)+t^2\mathcal D_{dec}(\theta)+tu\,\mathcal C_{dec}'(\theta)+\frac12tu^2\mathcal C_{dec}''(\theta)+O(t^3).
\end{align*}
where the omitted terms have expectation \(O(t^3)\) because the first neglected contributions are of the forms \(t^2u\), \(tu^3\), and \(t^2u^2\). Taking expectations gives
\begin{align*}
    \mathbb E[\widehat\theta]-\theta
    =& t\mathcal C_{dec}(\theta)+t^2\mathcal D_{dec}(\theta)+\mathbb E[u]+t\mathbb E[u]\mathcal C_{dec}'(\theta)+\frac12 t\mathbb E[u^2]\mathcal C_{dec}''(\theta)+O(t^3) \\
    =& t[\mathcal C_{enc}(\theta)+\mathcal C_{dec}(\theta)] \\
    &+ t^2\left[\mathcal D_{dec}(\theta)+\mathcal C_{enc}(\theta)\mathcal C_{dec}'(\theta)+\mathcal D_{enc}(\theta)+\frac12\mathcal A(\theta)\mathcal C_{dec}''(\theta)\right]+O(t^3).
\end{align*}
In particular,
\begin{equation}
...
\begin{itemize}
    ...
    Write \(p=2q\), so that \(q\in\mathbb{N}\) and the powers \(2q-1\) appearing below are exactly those associated with the even loss exponent \(p\).
    ...
    To solve for $\widehat{\theta}$, we expand -- taking without loss of generality both $\theta$ and  $F(\theta)$ to be $0$ here:
    %\reviewNotes{The notation in this even-\(p\) derivation is not internally consistent. Here \(m\) denotes the sensory observation, but the next line writes \(F^{-1}(x)=m+Mx+Nx^2+\cdots\), whose constant term should have stimulus units, not sensory-coordinate units. If the expansion variable is centered at \(m\), one would expect something like \(F^{-1}(m+x)=F^{-1}(m)+Mx+Nx^2+\cdots\). As written, the subsequent integral and the identifications of \(M,N,P,Q\) are difficult to audit because the roles of \(m\), \(x\), and the centering point are blurred.}
\begin{align*}
r=&Rt+St^2+O(t^3) \\
F^{-1}(x)=& Mx + Nx^2 +Px^3 + Qx^4 + O(x^5) \\
q_{prior}(x) =& q_{prior}(m) + Tx + Ux^2 + Vx^3 + O(x^4)
\end{align*}
and then set (dropping higher-order terms without further indication in the second equality):
\begin{align*}
    ...
    &= \sum \binom{2q-1}{n_1n_2n_3n_4n_5n_6} (-1)^{n_3+n_4+n_5+n_6} R^{n_1}S^{n_2}M^{n_3}N^{n_4}P^{n_5}Q^{n_6}t^{n_1+2n_2} \\
&\ \ \ \ \ \ \  \ \ \ \ \int x^{n_3+2n_4+3n_5+4n_6}\frac{1}{\sqrt{t}}
...
\begin{align*}
    0 &= \\
    &[\frac{(2q-1)!}{(2q-2)!1!}SM^{2q-2}q_{prior}(m)(2q-3)!! - \frac{(2q-1)!}{(2q-2)!1!}QM^{2q-2}q_{prior}(m)(2q-3)!! \\
& + \frac{(2q-1)!}{(2q-4)!2!}RM^{2q-4}N^2q_{prior}(m)(2q-1)!!\\
    &+\frac{(2q-1)!}{(2q-4)!3!}R^3M^{2q-4}q_{prior}(m)(2q-5)!! - \frac{(2q-1)!}{(2q-4)!2!}R^2M^{2q-4}Nq_{prior}(m)(2q-3)!! \\
& - \frac{(2q-1)!}{(2q-4)!3!}M^{2q-4}N^3q_{prior}(m)(2q+1)!!\\
...
\begin{align*}
    ...
    &+ \frac{(2q-2)(2q-1)(2q+1)}{2}M^{-1}N^2Tq^{-1}_{prior}(m) - (2q-1)RUq^{-1}_{prior}(m) + (2q+1)MVq^{-1}_{prior}(m)\\
    &- \frac{2q-2}{6}R^3M^{-2} + \frac{(2q-3)(2q-2)}{2}R^2M^{-2}N + \frac{(2q-3)(2q-2)(2q-1)(2q+1)}{6}M^{-2}N^3\\
    &- (2q-2)(2q-1)RM^{-1}NTq^{-1}_{prior}(m) + (2q-1)(2q+1)PTq^{-1}_{prior}(m) \\
& + (2q-1)(2q+1)NUq^{-1}_{prior}(m) + O(t)
\end{aligned}
\end{equation}
This $S$ is equal to the desired $\mathcal{D}_{dec}$.
We need to unpack the terms in the above expression to check that dependence on exponent, encoding, and prior is as claimed.
First, in the expansion
\[
F^{-1}(x)=\theta+Mx+Nx^2+Px^3+Qx^4+O(x^5)
\]
the individual coefficients are
\begin{align*}
M &= \frac{1}{F'},\\
N &= -\frac{F''}{2F'^3},\\
P &= -\frac{F'''}{6F'^4}+\frac{F''^2}{2F'^5},\\
Q &= \frac{-F''''F'^2+10F'F''F'''-15F''^3}{24F'^7}.
\end{align*}
We next consider the expansion 
\[
q_{prior}(x)=\frac{\Prior(F^{-1}(x))}{F'(F^{-1}(x))}=q_0+Tx+Ux^2+Vx^3+O(x^4),
\]
Write \(L\) for \(\log\Prior\), and $q_0 := q_{prior}(0) = \frac{\Prior(\theta)}{F'(\theta)}$. Define
\begin{align*}
A_1 &:= L' - \frac{F''}{F'},\\
A_2 &:= L'' - \frac{F'''}{F'} + \frac{F''^2}{F'^2},\\
A_3 &:= L''' - \frac{F''''}{F'} + 3\frac{F''F'''}{F'^2} - 2\frac{F''^3}{F'^3},
\end{align*}
where all derivatives are evaluated at the same \(\theta\). 
Then the coefficients are
\begin{align*}
T &= q_0\frac{A_1}{F'},\\
U &= q_0\left[\frac{A_2+A_1^2}{2F'^2}-\frac{F''A_1}{2F'^3}\right],\\
V &= q_0\left[\frac{A_3+3A_1A_2+A_1^3}{6F'^3}-\frac{F''(A_2+A_1^2)}{2F'^4}+\left(-\frac{F'''}{6F'^4}+\frac{F''^2}{2F'^5}\right)A_1\right].
\end{align*}
Finally, the coefficient $R$ corresponds to the first-order coefficient of the decoding bias computed in \citet{hahn2024unifying}; it can here be written as
\begin{align*}
R
&= \frac{F' L' - \frac{2q+1}{2}F''}{F'^3}
= \frac{A_1}{F'^2}-\frac{2q-1}{2}\frac{F''}{F'^3}.
\end{align*}
Now we substitute these identities into Eq.~\eqref{eq:d-even-p}. Every occurrence of \(q_{prior}(m)^{-1}\) is canceled by the prefactors in \(T,U,V\), so the result is a polynomial in
\begin{align*}
&L'=(\log\Prior)'(\theta),\quad L''=(\log\Prior)''(\theta),\quad L'''=(\log\Prior)'''(\theta),\\
&\quad F',\quad F'',\quad F''',\quad F'''',\quad \frac1{F'},\quad \frac1{\Prior(\theta)}.
\end{align*}
Also, Eq.~\eqref{eq:d-even-p} contains at most cubic prefactors in \(2q\pm1\), while \(R\) is affine linear in \(q\).
Thus, \(\mathcal D_{dec}\) is polynomial in \(q\) (hence, in $p=2q$) of degree at most \(4\).
...
We work at a general stimulus \(\theta\), set \(m=F(\theta)\), and define
\[
Q(\theta):=\log P_{prior}(\theta), \qquad \mathcal{S}(\theta):=F'(\theta)^2,
\]
and write the MAP estimate as \(\widehat{\theta}=\theta+r\). Then
    \begin{align*}
    \frac{d}{d r} \log P(\theta+r|m) &= 0\\
\end{align*}
We thus have
\begin{align*}
    0 &= t \frac{d}{d r} \log P(\theta+r|m)\\
    &= t \frac{d}{d r} \log P(m|\theta+r) + t \frac{d}{d r} Q(\theta+r)\\
    &= -F'(\theta+r) \cdot (F(\theta+r) - m) + t Q'(\theta+r)\\
    &= -\mathcal{S}(\theta) \cdot r - \frac{3}{2}F'(\theta)F''(\theta) \cdot r^2 + t Q'(\theta) + trQ''(\theta) + O(r^3)\\
\end{align*}
Rearranging,
\begin{align*}
    \mathcal{S}(\theta) \cdot r &= - \frac{3}{2}F'(\theta)F''(\theta) \cdot r^2 + t Q'(\theta) + trQ''(\theta) + O(r^3)\\
    ...
    We thus obtain
    \begin{align*}
    r &= - \frac{3F''(\theta)}{2F'(\theta)} \cdot r^2 + t \frac{Q'(\theta)}{\mathcal{S}(\theta)} + t \frac{Q''(\theta)}{\mathcal{S}(\theta)} r + O(r^3)\\
\end{align*}
Since \(r\) is of order \(O(t)\), we let \(r = Pt + Qt^2 + O(t^3)\) and plug back in the equation. 
By comparing the coefficients of the same order of \(t\), we can get the refined bias:
\begin{align*}
    r &= Pt + Qt^2 + O(t^3)\\
    &= - \frac{3F''(\theta)}{2F'(\theta)} \cdot (Pt + Qt^2 + O(t^3))^2 + t \frac{Q'(\theta)}{\mathcal{S}(\theta)} + t \frac{Q''(\theta)}{\mathcal{S}(\theta)} (Pt + Qt^2 + O(t^3)) + O(t^3)\\
    &= - \frac{3F''(\theta)}{2F'(\theta)} \cdot P^2t^2 + t \frac{Q'(\theta)}{\mathcal{S}(\theta)} + Pt^2 \frac{Q''(\theta)}{\mathcal{S}(\theta)} + O(t^3).
    \end{align*}
    We obtain the terms of the expansion:
    \begin{align*}
    P &= \frac{Q'(\theta)}{\mathcal{S}(\theta)}\\
    Q &= - \frac{3F''(\theta)}{2F'(\theta)} \cdot P^2 + P \frac{Q''(\theta)}{\mathcal{S}(\theta)}\\
    &= -\frac{3F''(\theta)}{2F'(\theta)} \cdot \frac{Q'(\theta)^2}{\mathcal{S}(\theta)^2} + \frac{Q'(\theta)Q''(\theta)}{\mathcal{S}(\theta)^2}.
\end{align*}
Thus
\begin{align*}
\mathcal C_{dec}^{MAP}(\theta) &= \frac{Q'(\theta)}{F'(\theta)^2},\\
\mathcal D_{dec}^{MAP}(\theta) &= \frac{Q'(\theta)\left(2F'(\theta)Q''(\theta)-3F''(\theta)Q'(\theta)\right)}{2F'(\theta)^5}.
\end{align*}
    ...
    \begin{align*}
        ...
        q_{prior}(x) &= q_{prior}(m) + Tx + Ux^2 + Vx^3 + O(x^4)
    \end{align*}
For \(p=1\), the estimator is the posterior median. Let \(x_\ast\) be defined by
\[
F^{-1}(x_\ast)=F^{-1}(m)+r.
\]
Then
\[
x_\ast = \frac{r}{M} - \frac{N}{M^3}r^2 + O(r^3).
\]
According to the same Taylor-expansion approach as in \citet{hahn2024unifying}, and applying it to the posterior median condition
\[
\int_{-\infty}^{x_\ast} q_{prior}(x)\frac{1}{\sqrt{2\pi t}}e^{-x^2/(2t)}dx
=
\frac12\int_{-\infty}^{\infty} q_{prior}(x)\frac{1}{\sqrt{2\pi t}}e^{-x^2/(2t)}dx,
\]
we have, writing \(s:=x_\ast/\sqrt{t}\),
\begin{align*}
0
&=
q_{prior}(m)\bigl(2\Phi(s)-1\bigr)-2T\sqrt{t}\,\phi(s)\\
&\qquad + Ut\bigl(2\Phi(s)-1-2s\phi(s)\bigr)-2Vt^{3/2}(s^2+2)\phi(s)+O(t^{5/2})\\
&=
\sqrt{\frac{2}{\pi}}
\left[
q_{prior}(m)s
-T\sqrt{t}
-\frac{q_{prior}(m)}{6}s^3
+\frac{T}{2}\sqrt{t}\,s^2
-2Vt^{3/2}
\right]
+O(t^{5/2}),
...
where we used
\[
2\Phi(s)-1=\sqrt{\frac{2}{\pi}}\left(s-\frac{s^3}{6}\right)+O(s^5),
\qquad
\phi(s)=\frac{1}{\sqrt{2\pi}}\left(1-\frac{s^2}{2}\right)+O(s^4).
\]
Let \(r = Rt + St^2 + O(t^3)\). Then
\[
s=\frac{R}{M}\sqrt{t}
+\left(\frac{S}{M}-\frac{NR^2}{M^3}\right)t^{3/2}
+O(t^{5/2}),
\]
and comparing the coefficients of \(\sqrt{t}\) and \(t^{3/2}\), we get
\begin{align*}
R &= \frac{MT}{q_{prior}(m)},\\
S &= \frac{2MV}{q_{prior}(m)} + \frac{NR^2}{M^2} + \frac{R^3}{6M^2} - \frac{TR^2}{2Mq_{prior}(m)}\\
&= \frac{2MV}{q_{prior}(m)} + \frac{NT^2}{q_{prior}(m)^2} - \frac{MT^3}{3q_{prior}(m)^3}.
\end{align*}
The lowest-order term is
\begin{align*}
R
= \frac{1}{\mathcal{S}}\frac{d}{d \theta} \log \frac{P_{prior}(\theta)}{\sqrt{\mathcal{S}(\theta)}},
\end{align*}
using \(M=(F^{-1})'(m)=1/\sqrt{\mathcal S(\theta)}\) and
\[
T = \frac{d}{dm}q_{prior}(m) = M\frac{d}{d\theta}q_{prior}(\theta),
\qquad
q_{prior}(\theta)=\frac{P_{prior}(\theta)}{\sqrt{\mathcal S(\theta)}},
\]
and this gives the desired \(\mathcal D_{dec}^{L_1}=S\).
\end{itemize}
Combining the explicit \(p=0\) and \(p=1\) formulas for \(\mathcal D_{dec}\) with Eq.~\ref{eq:dm-decomposition} shows that \(\mathcal D_M\) depends, in these two cases, on
\[
(\log \Prior)'(\theta),\quad (\log \Prior)''(\theta),\quad (\log \Prior)'''(\theta),\quad F'(\theta),\quad F''(\theta),\quad F'''(\theta),\quad F''''(\theta),\quad \frac1{F'(\theta)}.
\]
In the case of $p=2,4,6,\dots$, the same dependence follows from Eq.~\ref{eq:d-even-p}, with affine linear dependence on \((\log \Prior)'''(\theta)\).
\end{proof}
...
\]
for \(t \geq 0\), \(z \in \mathcal{Y}\), \(y \in \mathcal{X}\), and
\begin{equation}
...
\begin{align*}
f(t,z,y)=& Q(z,y) + \frac{t}{2} Q_{xx}(z,y) + \frac{t^2}{8} Q_{xxxx}(z,y)+\cdots \\
\end{align*}
We further substitute $y = F^{-1}(z) + t R$:
\begin{align*}
f(t,z,F^{-1}(z) + t R)=& Q(z,F^{-1}(z) + t R) + \frac{t}{2} Q_{xx}(z,F^{-1}(z) + t R) + \frac{t^2}{8} Q_{xxxx}(z,F^{-1}(z) + t R)+\cdots 
\end{align*}
...
\end{equation}
where $u$ is bounded as $t \rightarrow 0$, we want to show
...
For instance, \cite{hahn2024unifying} found that, in a direction perception experiment conducted by \citet{gekas2013complexityas}, subjects rapidly adapted their prior to a between-subjects manipulation of the experimental distribution, whereas the encoding reflected the pattern expected from efficient coding for environmental distribution.
...
We rigorously underpin this strategy in the case where the encoding stays constant, and the prior adapts to reflect some environmental statistics. This will almost surely result in identifiability, at least when the encoding resources are not fully uniform:
\begin{thm}[Prior adapts, encoding stays constant]\label{theorem:only-prior-adapts}
    For any given encoding $F$ such that encoding resources are not fully uniform (i.e., $F''$ is not identically zero), the set of priors $\Prior$ such that
        \begin{equation}
...
\end{thm}
\begin{proof}
This follows from the same proof as of Lemma~\ref{lemma:measure-zero}: 
That proof conditioned on $F$ and $p$ to show that the set of priors resulting in $\langle F', \Prior, p\rangle \in \Omega$ has measure zero.
The claim here then results from summing over the finitely many $p \in \mathcal{P}$.
\end{proof}

It is also conceivable that  encoding and prior \emph{jointly} adapt to stimulus statistics, as suggested by theories of efficient coding \citep[e.g.][]{Wei2015ABO, Morais2018PowerlawEN}.
In this case, prior and encoding would be systematically linked, such as via a power law: $\sqrt{\mathcal{J}(\theta)} \propto \Prior(\theta)^\alpha$ for some exponent $\alpha$ \citep[e.g.][]{Wei2015ABO, Morais2018PowerlawEN}.
We study this setting via simulations in Section~\ref{sec:simulations-efficient-coding}.
