...
\begin{equation}\label{eq:l1-loss}
    \int_{\mathcal{X}} |x-\theta| P(x|m) dx
\end{equation}
In the implementation of \citet{hahn2024unifying}, we are given the discretized posterior $P(x_1|m), \dots, P(x_N|m)$ based on the discretization $x_1, ..., x_N$ of the stimulus space $\mathcal{X}$ ($x_{i+1}-x_i = \Gamma$).
...
\begin{align}\label{eq:interval-l1-estimator}
     \arg\min_{\theta \in \mathcal{X}} \int_{\mathcal{X}} |\theta-x| \widehat{P}(x) dx 
\end{align}
\end{defin}
...
\end{align*}
We now want to choose $\alpha \in [-\frac{1}{2},\frac{1}{2}]$ such that $\widehat{\theta} = x_a + \alpha\Gamma$.
Then, $\widehat{\theta}$ partitions the patch $[x_a-\frac{\Gamma}{2}, x_a+\frac{\Gamma}{2}]$ with length $\Gamma$ into sub-patches of length $(\frac{1}{2}+\alpha) \Gamma$ (on the left) and $(\frac{1}{2}-\alpha) \Gamma$ (on the right).
The overall mass on the left of $\widehat{\theta}$ is
\begin{equation}
  \int_{x<\widehat{\theta}} \widehat{P}(x) dx  =  \Gamma C + \left(\frac{1}{2}+\alpha\right) \Gamma A %+ (\frac{1}{2}-\alpha) \Gamma B
\end{equation}
The overall mass on the right of $\widehat{\theta}$ is
\begin{equation}
  \int_{x>\widehat{\theta}} \widehat{P}(x) dx =   \Gamma D + \left(\frac{1}{2}-\alpha\right) \Gamma A %+ (\frac{1}{2}+\alpha) \Gamma B
\end{equation}
...
Then
\begin{equation}
    \frac{\partial \widehat{\theta}}{\partial P(x_i)} = \frac{\partial \alpha}{\partial P(x_i)} \cdot \Gamma
\end{equation}
...
\begin{equation}
    \phi_i(x) = \begin{cases}
        1 & \text{if } \cos(x-x_i) > \cos(\frac{\Gamma}{2}) \\
        0 & \text{else}
    \end{cases}
\end{equation}
We first set
...
\end{align*}
We now want to choose $\alpha \in [-\frac{1}{2},\frac{1}{2}]$ such that $\widehat{\theta} = x_a + \alpha\Gamma$.
Then, among the patch $[x_a-\frac{\Gamma}{2}, x_a+\frac{\Gamma}{2}]$ with length $\Gamma$, it is partitioned into sub-patches of length $(\frac{1}{2}+\alpha) \Gamma$ (on the left) and $(\frac{1}{2}-\alpha) \Gamma$ (on the right).
The overall mass on the left of $\widehat{\theta}$ is
\begin{equation}
  \int_{x : sin(x-\widehat{\theta}) < 0} \widehat{P}(x) dx =  \Gamma D + \left(\frac{1}{2}+\alpha\right) \Gamma A + \left(\frac{1}{2}-\alpha\right) \Gamma B
\end{equation}
The overall mass on the right of $\widehat{\theta}$ is
\begin{equation}
  \int_{x : sin(x-\widehat{\theta}) > 0} \widehat{P}(x) dx =  \Gamma C + \left(\frac{1}{2}-\alpha\right) \Gamma A + \left(\frac{1}{2}+\alpha\right) \Gamma B
\end{equation}
Setting these two equal results in
...
We compare four numerical methods of computing or approximating the Fisher information in Figure~\ref{fig:four-methods-fi}:
\begin{enumerate}
...
\begin{equation}
    \sqrt{\FI(\theta)} = \sqrt{4\pi} \frac{\mathbb{P}\left(\sin(m(\theta_{i+1})- m(\theta_i)) > 0\right) + \frac{1}{2}\mathbb{P}\left(m(\theta_{i+1}) = m(\theta_i)\right) - 0.5}{\theta_{i+1}-\theta_i}
\end{equation}
