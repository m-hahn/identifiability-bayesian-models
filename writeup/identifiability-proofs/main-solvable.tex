...
Remarkably, we will show that this model family\footnote{Strictly speaking, restrictions of this model to a finite stimulus space interval. For any finite interval, one obtains an element of $\mathfrak{M}$ by restricting prior and $F$ to this interval, and rescaling $F$ to ensure $\int F' = 1$.} is entirely contained in the exceptional set $\Omega$ studied in Theorem 2, and, indeed, \emph{no number of sensory noise levels} will be sufficient for disentangling prior and loss function in this model family. An important implication is thus that $\Omega$, even though it is very small in a mathematically precise sense, may still contain such very natural models.
...
Let $\mu, \sigma^2 > 0$ be the parameters of a log-normal prior. Let $t$ denote the variance of sensory noise.
    The family of models
    \begin{equation}\label{eq:lognormal-family}
      \{  \left\langle F'(\theta) = \frac{1}{\theta}, \Prior = LogNormal\left(\log \mu - \frac{\tilde{p}}{2}\sigma^2, \sigma^2\right), p\right\rangle : p =0, 1, 2, 4, \dots\}
    \end{equation}
...
\begin{proof}
    ...
    \begin{equation}
        p(m|\theta) \propto LogNormal(\log \theta, t) := \frac{1}{m} \exp\left(-\frac{(\log m - \log \theta)^2}{2t}\right).
\end{equation}
...
\begin{lemma}
\begin{align*}
  G(\theta) :=  \Prior(\theta) F'(\theta)^{\frac{\tilde{p}-\tilde{p}'}{2}} \propto LogNormal\left(\log\mu - \frac{\tilde{p}-\tilde{p}'}{2}\sigma^2, \sigma^2\right)
\end{align*}
\end{lemma}
\begin{proof}
Define
\[
\delta = \frac{\tilde{p}-\tilde{p}'}{2}.
\]
...
\]
Notice that \(\delta \log \theta = \frac{\tilde{p}-\tilde{p}'}{2}\log \theta\). %\textcolor{blue}{Here should be \( \frac{p-p'}{2}\log \theta \)} 
...
\[
G(\theta) \propto \frac{1}{\theta}\exp\left[-\frac{\left(\log \theta - \left(\log \mu - \delta\sigma^2\right)\right)^2}{2\sigma^2}\right] = LogNormal\left(\log\mu - \frac{\tilde{p}-\tilde{p}'}{2}\sigma^2, \sigma^2\right).
\]
\end{proof}
