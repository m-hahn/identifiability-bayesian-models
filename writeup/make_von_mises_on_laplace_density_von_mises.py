...
def plot_density_panels():
    ...
    for condition in NOISE_CONDITIONS:
        ...
        sensory_likelihoods_vonMises = torch.softmax(
            ((SQUARED_SENSORY_SIMILARITY(F[:-1].unsqueeze(0) - F[:-1].unsqueeze(1))) / (sigma2))
            + volumeElement.unsqueeze(1).log(),
            dim=0,
        )

        x_axis, density = orientation_axis_and_density(grid, reference_index, sensory_likelihoods_vonMises[:, reference_index])
    ...
    axis.set_ylabel("Von Mises density", fontsize=AXIS_LABEL_FONTSIZE)
    ...
    figure.tight_layout()
    figure.savefig(FIGURES_DIR / "von_mises_on_laplace_density_von_mises.pdf", bbox_inches="tight")
    plt.close(figure)
