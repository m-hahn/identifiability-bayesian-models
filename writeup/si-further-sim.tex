...
To illustrate this, throughout the SI Appendix, we visualize not only the fitted overall bias, but also the attraction and repulsion components.
...
To achieve this, we follow \cite{hahn2024unifying} in operationalizing the attraction component as the decoding bias ($\mathbb{E}[{\widehat{\theta}}|m] - F^{-1}(m)$) for the MAP estimator ($p=0$), which has the analytical form
\begin{equation}
    \frac{1}{\FI(\theta)} (\log \Prior(\theta))' + \mathcal{O}(\sigma^4)
\end{equation}
matching the Prior Attraction up to a higher-order residual.
Also following \cite{hahn2024unifying}, we operationalize the repulsion component as the difference between the overall bias and this component.
...
\subsection{Models on Circular Spaces (Supplement to Figure 5)}\label{sec:further-circular}
...
\begin{equation}
\sum_{i=0}^4 \left( X_i \sin(i \theta) + Y_i \cos(i \theta) \right)
\end{equation}
