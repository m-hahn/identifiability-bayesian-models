...
FILE = f"logs/CROSSVALID/{__file__.replace('_VIZ', '')}_{FIT_OUT}_{P}_{FOLD_HERE}_{REG_WEIGHT}_{GRID}.txt"
print(FILE)
...
#     axis.plot([float(sample.min()), float(sample.max())], 2*[0.5*float(prior.max())], color="orange")
     ...
     savePlot(f"figures/{__file__}_{FIT_OUT}_{P}_{FOLD_HERE}_{REG_WEIGHT}_{GRID}_Prior.pdf")
