...
class MAPIntervalEstimator(torch.autograd.Function):
    ...
    def set_parameters(**kwargs):
        ...
        assert MAX_UPDATE_FACTOR == 5
        global BREAK_THRESHOLD
        BREAK_THRESHOLD = 1e-5
        if "BREAK_THRESHOLD" in kwargs:
           BREAK_THRESHOLD = kwargs["BREAK_THRESHOLD"]
        assert BREAK_THRESHOLD == 1e-5

        # we need to ensure consistency
        assert W == 800
#        assert KERNEL_WIDTH == 0.01

        W = 1600
        assert KERNEL_WIDTH == 0.1
...
#        print(grid_indices_here[:,0])
 #       quit()
        #assert posterior.min() > -1e5
        #assert posterior.min() < 1e5

        kernelFunction = torch.nn.functional.softmax(-(grid_indices_here[::10,::10] - grid_indices_here[::10,::10].t()).pow(2) / (2*(KERNEL_WIDTH**2)), dim=0)
    #    print(kernelFunction.size(), posterior.size())
        smoothedPosterior = (kernelFunction.view(GRID//10, GRID//10, 1) * posterior[::10].view(1, GRID//10, GRID)).sum(dim=0)
        resultDiscrete = 10*smoothedPosterior.argmax(dim=0)
        resultDiscrete = torch.stack([grid_indices_here[resultDiscrete[i],i] for i in range(GRID)], dim =0)
#   #     print(resultDiscreteFromRaw)       
  #      print(resultDiscrete)
 #       quit()
...
#           assert smoothedPosteriorInEnvironment.min() > -1e5, smoothedPosteriorInEnvironment
#
...
#           #        assert smoothedPosteriorInEnvironment.min() > -1e5
...
#        resultDiscrete = resultDiscrete.clamp(min=MIN_GRID/SCALE, max=MAX_GRID/SCALE)
        ...
        if True:
          for i in range(200): #80):
             ...
             # We can clamp it, to make things easier for the optimizer. The minimum will be slightly inside the boundary
             #resultDiscrete = resultDiscrete.clamp(min=MIN_GRID/SCALE, max=MAX_GRID/SCALE)
...
#            axis[1].scatter(SCALE*grid_indices_here[:,worst].detach().cpu(), derivativeSmoothedPosteriorOverAll[:,worst].detach().cpu(), color="purple")
            ...
            Z = max(0.1, max(float(smoothedPosteriorOverAll[:,0].max()), float(smoothedPosteriorOverAll[:,worst].max())))
            Y = min(float(smoothedPosteriorOverAll[:,0].min()), float(smoothedPosteriorOverAll[:,worst].min()))
            assert smoothedPosteriorOverAll.min() > -1e5
            print("SMOOTHED POSTERIOR", smoothedPosteriorOverAll[:,worst])
            print(smoothedPosteriorOverAll[:,worst].max())
            print(smoothedPosteriorOverAll[:,worst].min())
            print(smoothedPosteriorOverAll[:,0].max())
            print(smoothedPosteriorOverAll[:,0].min())
            for x, y, z, u, mask in optimizationSequence:
               print("OPTIM SEQUENCE", x[0], x[worst], "P", y[worst], "dP", z[worst], "d2P", u[worst], derivativeSmoothedPosterior[worst], resultDiscrete[worst], mask[worst])
               axis[0].plot([SCALE*x[0].detach().cpu(), SCALE*x[0].detach().cpu()], [-Z,Z], color="yellow", alpha=0.1)
               axis[1].plot([SCALE*x[worst].detach().cpu(), SCALE*x[worst].detach().cpu()], [-Z,Z], color="yellow", alpha=0.1)
            ...
            axis[0].set_xlim(-0.2, 3.2)
            axis[1].set_xlim(-0.2, 3.2)
            axis[0].set_ylim(-0.5*Z,Z)
            axis[1].set_ylim(-0.5*Z,Z)
            assert Z > 0, Z
#            savePlot(f"figures/DEBUG_{__file__}.pdf")
            plt.show()
            plt.close()
