...
with open(f"output/{__file__}_{FIT}.tex", "w") as outFile:
 for loss in [0,1,2,4,6,8]:
    if loss == 0:
       full_freeprior = crossValidResults(f"Interval/RunSynthetic_DenseRemington_FreeEncoding_Zero_OnSim_OtherNoiseLevels_VarySize_Round2.py_{FIT}_{loss}_*_{REG}_{GRID}.txt", STRICT=False)
    elif loss == 1:
       full_freeprior = crossValidResults(f"Interval/RunSynthetic_DenseRemington_FreeEncoding_L1_OnSim_OtherNoiseLevels_VarySize_Round2.py_{FIT}_{loss}_*_{REG}_{GRID}.txt", STRICT=False)
    else:
       full_freeprior = crossValidResults(f"Interval/RunSynthetic_DenseRemington_FreeEncoding_OnSim_OtherNoiseLevels_VarySize_Round2.py_{FIT}_{loss}_*_{REG}_{GRID}.txt", STRICT=False)
    ...
    print(f"RunSynthetic_DenseRemington_FreeEncoding_OnSim_OtherNoiseLevels_VarySize.py_{FIT}_{loss}_*_{REG}_{GRID}.txt")
    print(f"RunSynthetic_DenseRemington_FreeEncoding_OnSim_OtherNoiseLevels_VarySize.py_{FIT}_{RELEVANT_EXP}_*_{REG}_{GRID}.txt")

    if RELEVANT_EXP == 0:
       reference = crossValidResults(f"Interval/RunSynthetic_DenseRemington_FreeEncoding_Zero_OnSim_OtherNoiseLevels_VarySize_Round2.py_{FIT}_{RELEVANT_EXP}_*_{REG}_{GRID}.txt", STRICT=False)
    elif RELEVANT_EXP == 1:
       reference = crossValidResults(f"Interval/RunSynthetic_DenseRemington_FreeEncoding_L1_OnSim_OtherNoiseLevels_VarySize_Round2.py_{FIT}_{RELEVANT_EXP}_*_{REG}_{GRID}.txt", STRICT=False)
    else:
       reference = crossValidResults(f"Interval/RunSynthetic_DenseRemington_FreeEncoding_OnSim_OtherNoiseLevels_VarySize_Round2.py_{FIT}_{RELEVANT_EXP}_*_{REG}_{GRID}.txt", STRICT=False)

    print("LOSS", full_freeprior)
    print("REFERENCE", reference, f"RunSynthetic_DenseRemington_FreeEncoding_OnSim_OtherNoiseLevels_VarySize_Round2.py_{FIT}_{RELEVANT_EXP}_*_10.0_400.txt")
...
for key, values in curvesRelativeLF.items():
    ...
    color = "gray"
    axis.plot(x, y, color=COLOR, linestyle={1 : 'solid', 4 : "dotted"}[NUMBER], linewidth=0.5)
