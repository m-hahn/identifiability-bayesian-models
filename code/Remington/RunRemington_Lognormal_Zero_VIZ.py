...
def model(grid):
  ...
  for iteration in range(1):
   ...
   if iteration % 500 == 0:
     ...
     figure, axis = plt.subplots(1, len(gridspec["width_ratios"]), figsize=(0.8*16,0.8*2.5), gridspec_kw=gridspec)
   ...
   if iteration % 500 == 0:
     ...
     axis.plot([grid_cpu[0], grid_cpu[-1]], [0,0], color="white")
     ...
     axis.violinplot((bayesianEstimate_model-grid-attraction).detach().abs(), positions=[1], showextrema=False)
     plt.xticks([])
     axis.spines['top'].set_visible(False)
     axis.spines['right'].set_visible(False)
     axis.set_yticks(ticks=[0,0.2,0.4,0.6], labels=[0, "", "0.4", ""], fontsize=15)
     ...
     figure, axis = plt.subplots(1, 1, figsize=(2,2))
     figure.subplots_adjust(left=0.25, bottom=0.22)
     ...
     createWeightedScatter(axis, 0, attraction.detach().abs()[MASK], c=(30/255, 117/255, 179/255, 76/255))
     createWeightedScatter(axis, 1, (bayesianEstimate_model-grid-attraction).detach().abs()[MASK], c=(255/255, 125/255, 12/255, 76/255))
     axis.violinplot((attraction).detach().abs()[MASK], positions=[0], showextrema=False)
     axis.violinplot((bayesianEstimate_model-grid-attraction).detach().abs()[MASK], positions=[1], showextrema=False)
     plt.xticks([])
     axis.set_xlim(-0.5, 1.5)
     axis.spines['top'].set_visible(False)
     axis.spines['right'].set_visible(False)
     axis.set_yticks(ticks=[0,0.05, 0.1], labels=[0, "0.05", ""], fontsize=15)
     ...
     plt.yticks([])
     axis.set_xticks(ticks=[0.5, 1.0], labels=["0.5", "1.0"], fontsize=15)
