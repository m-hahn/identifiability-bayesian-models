...
if len(glob.glob(f"losses/Interval/{__file__.replace('_VIZ', '')}_{FIT_OUT}_{P}_{FOLD_HERE}_{REG_WEIGHT}_{GRID}.txt.txt")) > 0:
    assert False, f"losses/Interval/{__file__.replace('_VIZ', '')}_{FIT_OUT}_{P}_{FOLD_HERE}_{REG_WEIGHT}_{GRID}.txt.txt"
...
print(FIT)
FIT_parsed = FIT_ #[:-4].split("_") #SimulateSynthetic2_DenseRemington_OtherNoiseLevels_VarySize.py_400_2_4567_N10000_UNIFORM_UNIMODAL.txt
PRIOR = FIT_parsed[-2]
...
#     plt.show()
   ...
   if iteration % 500 == 0 and iteration > 0:
       ...
       if len(lossesBy500) == 1 or float(loss) <= min(lossesBy500):
        with open(f"losses/Interval/{__file__.replace('_VIZ', '')}_{FIT_OUT}_{P}_{FOLD_HERE}_{REG_WEIGHT}_{GRID}.txt.txt", "w") as outFile:
           print(float(crossValidLoss), file=outFile)
        with open(f"logs/CROSSVALID/{__file__}_{FIT_OUT}_{P}_{FOLD_HERE}_{REG_WEIGHT}_{GRID}.txt", "w") as outFile:
