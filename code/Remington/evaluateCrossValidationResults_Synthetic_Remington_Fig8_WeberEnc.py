...
assert "SimulateRemington_Lognormal_OtherNoiseLevels_Zero" in FIT

REG = 0.1
GRID = 200
assert "Dense" not in FIT

with open(f"output/{__file__}_{FIT}.tex", "w") as outFile:
 for loss in [0,1,2,4,6,8]:
    if loss == 0:
       full_freeprior = crossValidResults(f"Interval/RunSynthetic_DenseRemington_WeberEncoding_Zero_OnSim_OtherNoiseLevels_VarySize_Round2.py_{FIT}_{loss}_*_{REG}_{GRID}.txt", STRICT=False)
    elif loss == 1:
       full_freeprior = crossValidResults(f"Interval/RunSynthetic_DenseRemington_WeberEncoding_L1_OnSim_OtherNoiseLevels_VarySize_Round2.py_{FIT}_{loss}_*_{REG}_{GRID}.txt", STRICT=False)
    else:
       full_freeprior = crossValidResults(f"Interval/RunSynthetic_DenseRemington_WeberEncoding_OnSim_OtherNoiseLevels_VarySize_Round2.py_{FIT}_{loss}_*_{REG}_{GRID}.txt", STRICT=False)
    ...
    assert "SimulateRemington_Lognormal_OtherNoiseLevels_Zero" in FIT
    print(FIT)
    RELEVANT_EXP = int(FIT.split(".py_")[1].split("_")[1])
    print("EXPONENT", int(RELEVANT_EXP))

    print(f"RunSynthetic_DenseRemington_WeberEncoding_OnSim_OtherNoiseLevels_VarySize.py_{FIT}_{loss}_*_{REG}_{GRID}.txt")
    print(f"RunSynthetic_DenseRemington_WeberEncoding_OnSim_OtherNoiseLevels_VarySize.py_{FIT}_{RELEVANT_EXP}_*_{REG}_{GRID}.txt")

    if RELEVANT_EXP == 0:
       reference = crossValidResults(f"Interval/RunSynthetic_DenseRemington_WeberEncoding_Zero_OnSim_OtherNoiseLevels_VarySize_Round2.py_{FIT}_{RELEVANT_EXP}_*_{REG}_{GRID}.txt", STRICT=False)
    elif RELEVANT_EXP == 1:
       reference = crossValidResults(f"Interval/RunSynthetic_DenseRemington_WeberEncoding_L1_OnSim_OtherNoiseLevels_VarySize_Round2.py_{FIT}_{RELEVANT_EXP}_*_{REG}_{GRID}.txt", STRICT=False)
    else:
       reference = crossValidResults(f"Interval/RunSynthetic_DenseRemington_WeberEncoding_OnSim_OtherNoiseLevels_VarySize_Round2.py_{FIT}_{RELEVANT_EXP}_*_{REG}_{GRID}.txt", STRICT=False)

    print("LOSS", full_freeprior)
    print("REFERENCE", reference, f"RunSynthetic_DenseRemington_WeberEncoding_OnSim_OtherNoiseLevels_VarySize_Round2.py_{FIT}_{RELEVANT_EXP}_*_{REG}_{GRID}.txt")
...
for key, values in curvesRelativeLF.items():
    ...
    color = "gray"
    linestylehere = {1 : 'solid', 4 : "dotted"}[NUMBER]
    if linestylehere == "dotted":
      linestylehere=(0, (10, 20))
    axis.plot(x, y, color=COLOR, linestyle=linestylehere, linewidth=0.5)
...
print(minY, maxY)
axis.set_xlim(-1,11)
axis.set_ylim(-4, 60) #minY-20, maxY+20)
...
with open(f"output/{__file__}.txt", "w") as outFile:
 for key, values in curvesRelativeLF.items():
    ...
    axis.plot(x, y, color=COLOR, linestyle={1 : 'solid', 4 : "dotted"}[NUMBER])
    axis.scatter(x, y, color=COLOR)
    #axis[i].errorbar(x, y, yerr=errors, color=color)
