...
FILE = f"logs/CROSSVALID/{__file__.replace('_VIZ_Figure8', '')}_{FIT}_{P}_{FOLD_HERE}_{REG_WEIGHT}_{GRID}.txt"
print(FILE)
...
def model(grid):
  ...
  for iteration in range(1):
   ...
   if iteration % 500 == 0:
     ...
     assigned = ["ENC", None, None, "PRI", None, None, "ATT", "REP", "TOT", None, None, "VAR", None, None, "HUM", None, None, "VAH"]
...
#     axis[PRI].plot([float(sample.min()), float(sample.max())], 2*[0], color="purple")
   ...
   if iteration % 500 == 0:
     ...
     if True:
        ...
        axis[ENC].text(-0.02, 55, "[1/sec]", fontsize=10)
        axis[VAR].text(-0.07, 0.23, "[sec]", fontsize=10)
#
#
     ...
     axis[ENC].set_yticks(ticks=[0, 10, 20, 30, 40, 50], labels=["0", "", "20", "", "40", ""])
     axis[PRI].set_yticks(ticks=[0])
     for z in [REP, ATT, TOT, HUM]:
