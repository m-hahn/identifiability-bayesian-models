...
if len(glob.glob(f"losses/Interval/{__file__.replace('_VIZ_OnlyModel', '')}_{FIT_OUT}_{P}_{FOLD_HERE}_{REG_WEIGHT}_{GRID}.txt.txt")) == 0:
    assert False, f"losses/Interval/{__file__.replace('_VIZ_OnlyModel', '')}_{FIT_OUT}_{P}_{FOLD_HERE}_{REG_WEIGHT}_{GRID}.txt.txt"

#assert "WEBER" in FIT or  "_400_0_4567_" in FIT, "for now focus on these"

assert "_pi_1_" in FIT
FIT_ = FIT.split("_")
assert "pi" == FIT_[-10], FIT_[-10]
FIT_ = FIT_[:-10]
print(FIT_)
#quit()
...
FILE = f"logs/CROSSVALID/{__file__.replace('_VIZ_OnlyModel', '')}_{FIT_OUT}_{P}_{FOLD_HERE}_{REG_WEIGHT}_{GRID}.txt"
print(FILE)
...
def model(grid):
  ...
  for iteration in range(1):
   ...
   if iteration % 500 == 0:
     assigned = ["PRI", None, "ENC", None, None, None]
     for w, k in enumerate(assigned):
         globals()[k] = w
     notToInclude = [None]
     PAD = [w for w, q in enumerate(assigned) if q in notToInclude]
     gridspec = dict(width_ratios=[1,0.5,1,0,0,0])
     figure, axis = plt.subplots(1, len(gridspec["width_ratios"]), figsize=(1.2*.4*6,.4*3), gridspec_kw=gridspec)
   ...
   for DURATION in range(10):
    ...
    for SUBJECT in [1]:
     ...
     if iteration % 500 == 0:
       resource_curve = computeResources(volume, torch.sigmoid(init_parameters["sigma_logit"][DURATION])).detach().cpu()
       max_resource = max(max_resource, float(resource_curve.max()))
       axis[ENC].plot(grid_cpu, resource_curve)

   if iteration % 500 == 0:
     axis[ENC].set_ylim(0, max(0.35, 1.05 * max_resource))
     for w in [ENC, PRI]:
       axis[w].spines['top'].set_visible(False)
       axis[w].spines['right'].set_visible(False)
       axis[w].set_xticks(ticks=[0, 1, 2, 3], labels=["0", "1", "2", "3"])
     ...
     savePlot(f"figures/{__file__}_{FIT_OUT}_{P}_{FOLD_HERE}_{REG_WEIGHT}_{GRID}.pdf")
