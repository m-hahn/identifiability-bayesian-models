...
class CosineEstimator(torch.autograd.Function):
    ...
    def forward(ctx, grid_indices_here, posterior):
        ...
        for itera in range(20):
          ...
          if False:
            loss = (((result.unsqueeze(0)/SCALE - grid_indices_here/SCALE).abs().pow(P)) * posterior.detach()).sum(dim=0).sum() / n_batch
          loss_gradient = ((P * torch.sign(result.unsqueeze(0)/SCALE - grid_indices_here/SCALE) * (result.unsqueeze(0)/SCALE - grid_indices_here/SCALE).abs().pow(P-1)) * posterior.detach()).sum(dim=0) / n_batch
          loss_gradient2 = ((P * (P-1) * (result.unsqueeze(0)/SCALE - grid_indices_here/SCALE).abs().pow(P-2)) * posterior.detach()).sum(dim=0) / n_batch
        ...
        for itera in range(20000):
          ...
          if True:
            loss = (((1-SQUARED_STIMULUS_SIMILARITY(result_.unsqueeze(0) - grid_indices_here)).pow(P/2)) * posterior.detach()).sum(dim=0).sum() / n_batch

          loss_gradient = (P/2 * (SQUARED_STIMULUS_DIFFERENCE(result_.unsqueeze(0) - grid_indices_here)) * ((1-SQUARED_STIMULUS_SIMILARITY(result_.unsqueeze(0) - grid_indices_here)).pow(P/2-1)) * posterior.detach()).sum(dim=0) / n_batch
          ...
          result += momentum
          print(itera, float(loss), "max absolute gradient after GD steps", loss_gradient.abs().max())
    ...
    def backward(ctx, grad_output):
        ...
        F = (P/2 * (SQUARED_STIMULUS_DIFFERENCE(result.unsqueeze(0) - grid_indices_here)) * ((1-SQUARED_STIMULUS_SIMILARITY(result.unsqueeze(0) - grid_indices_here)).pow(P/2-1)) * posterior.detach()).sum(dim=0)
        ...
        dF_posterior = P/2 * (SQUARED_STIMULUS_DIFFERENCE(result.unsqueeze(0) - grid_indices_here)) * ((1-SQUARED_STIMULUS_SIMILARITY(result.unsqueeze(0) - grid_indices_here)).pow(P/2-1))
        dF_result = 0
        dF_result = dF_result + (P/2 * (SQUARED_STIMULUS_SIMILARITY(result.unsqueeze(0) - grid_indices_here)) * ((1-SQUARED_STIMULUS_SIMILARITY(result.unsqueeze(0) - grid_indices_here)).pow(P/2-1)) * posterior.detach()).sum(dim=0)
        if P == 2:
           ...
           dF_result = dF_result + (P/2 * (P/2-1) * (SQUARED_STIMULUS_DIFFERENCE(result.unsqueeze(0) - grid_indices_here)).pow(2) * ((1-SQUARED_STIMULUS_SIMILARITY(result.unsqueeze(0) - grid_indices_here)).pow(P/2-2)) * posterior.detach()).sum(dim=0)
