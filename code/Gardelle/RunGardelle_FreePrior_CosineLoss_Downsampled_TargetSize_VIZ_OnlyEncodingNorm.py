...
FILE = f"logs/CROSSVALID/{__file__.replace('_VIZ_OnlyEncodingNorm', '')}_{P}_{FOLD_HERE}-{SEED}_{REG_WEIGHT}_{GRID}_{targetSize}_{levelsToDownSampleToString}.txt"
...
def model(grid):
  ...
  for iteration in range(1):
   ...
   if iteration % 100 == 0:
     ...
     axis[ENC].set_yticks(ticks=[0], labels=["0"])
     x_set = sorted(list(set(xValues.cpu().numpy().tolist())))
