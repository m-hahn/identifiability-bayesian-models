...
FILE = f"logs/CROSSVALID/{__file__.replace('_VIZ_EncPri', '')}_{P}_{FOLD_HERE}-{SEED}_{REG_WEIGHT}_{GRID}_{targetSize}_{levelsToDownSampleToString}.txt"
...
def model(grid):
  ...
  for iteration in range(1):
   ...
   if iteration % 100 == 0:
     ...
     figure, axis = plt.subplots(1, 9, figsize=(1.3*0.8*5/6*3,0.9*0.8*2), gridspec_kw=gridspec)
     plt.tight_layout()
