...
FILE = f"logs/CROSSVALID/{__file__.replace('_VIZ_OnlyEncodingTotalFI', '')}_{P}_{FOLD_HERE}-{SEED}_{REG_WEIGHT}_{GRID}_{targetSize}_{levelsToDownSampleToString}.txt"
...
def model(grid):
  ...
  for iteration in range(1):
   ...
   if iteration % 100 == 0:
     ...
     figure.tight_layout(rect=[0, 0.05, 1, 1])  # [left, bottom, right, top]
     ...
     axis[ENC].set_ylim(0, 30)
     axis[ENC].set_xlim(0, 6)
     axis[ENC].set_yticks(ticks=[0,10, 20,30])
     x_set = sorted(list(set(xValues.cpu().numpy().tolist())))
   ...
   for DURATION in levelsToDownSampleTo:
    for SUBJECT in [1]:
     ...
     if iteration % 100 == 0:
#       axis[ENC].plot([0,360], 2*computeResources(volume.detach(), inverse_variance=1/float((4 * torch.sigmoid(init_parameters["sigma_logit"][DURATION])))).sum() * MakeFloatTensor([1,1]), color=[None, "purple", "blue", "orange", "red", "green"][DURATION])
       times = [int(x) for x in ["0", "20", "40", "80", "160", "1000"]]
       axis[ENC].plot([math.log(times[DURATION]),math.log(times[DURATION])], [0,2*computeResources(volume.detach(), inverse_variance=1/float((4 * torch.sigmoid(init_parameters["sigma_logit"][DURATION])))).sum().item()], color=colors[DURATION-1],  linewidth=10)
...
#       axis[w].tick_params(labelbottom=False)
     ...
     axis[ENC].set_xlim(math.log(10),math.log(1200))
...
#       axis[ENC].set_title("Resources")
   ...
   for w in [ENC]:
#      ticktimes = [20, 40, 80, 160, 320, 640, 1280]
      ticktimes = [20, 80, 320, 1280]
      axis[w].set_xticks(ticks=[math.log(q) for q  in ticktimes], labels=[str(q) for q in ticktimes])
#   for w in range(9):
