...
FILE = f"logs/CROSSVALID/{__file__.replace('_VIZFig7', '')}_{P}_{FOLD_HERE}_{REG_WEIGHT}_{GRID}.txt"
...
def model(grid):
  ...
  for iteration in range(1):
   ...
   if iteration % 100 == 0:
     assigned = ["ENC", None, "PRI", None, "TOT", None, "HUM"]
     for w, k in enumerate(assigned):
         globals()[k] = w
     PAD = [w for w, q in enumerate(assigned) if q not in ["ENC", "PRI", "TOT","HUM"]]
     gridspec = dict(width_ratios=[1 if (x is not None and x not in PAD) else (.5 if x is None else 0.0) for x in assigned])
     figure, axis = plt.subplots(1, len(assigned), figsize=(8, 1.3*0.8*1.8), gridspec_kw=gridspec)
     plt.tight_layout()
     figure.subplots_adjust(wspace=0.1, hspace=0.0, top=1.0)
   ...
   for DURATION in range(1,6):
    for SUBJECT in [1]:
     ...
     if iteration % 100 == 0:
       ...
       CI_y1 = (.5*(y_errbar[:,0])).detach().numpy().tolist()
       CI_y2 = (.5*(y_errbar[:,1])).detach().numpy().tolist()
       poly = plt.Polygon(list(zip(CI_x + CI_x[::-1], CI_y1 + CI_y2[::-1])), facecolor=(.65, .65, .65), alpha=0.5)

       axis[HUM].add_patch(poly)
       axis[HUM].plot([float(grid[x]) for x in x_set], y_set/2, linewidth=0.8)
...
#     axis[REP].tick_params(labelleft=False)
     axis[HUM].tick_params(labelleft=False)
...
#     axis[ATT].set_ylim(-25, 25)
 #    axis[REP].set_ylim(-25, 25)
     ...
     if True:
       axis[PRI].set_title("") #Prior", fontsize=19)
       axis[ENC].set_title("") #Resources", fontsize=19)
       #axis[ATT].set_title("") #Attraction", fontsize=19)
       #axis[REP].set_title("") #Repulsion", fontsize=19)
       axis[TOT].set_title("") #Bias (model)", fontsize=19)
       axis[HUM].set_title("") #Bias (data)", fontsize=19)
