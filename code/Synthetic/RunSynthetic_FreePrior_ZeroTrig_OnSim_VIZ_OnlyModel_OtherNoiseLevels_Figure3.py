...
FILE = f"logs/CROSSVALID/{__file__.replace('_VIZ_OnlyModel_OtherNoiseLevels_Figure3', '')}_{FIT}_{P}_{FOLD_HERE}_{REG_WEIGHT}_{GRID}.txt"
...
def model(grid):
  ...
  for iteration in range(1):
   ...
   if iteration % 100 == 0:
     ...
     if P == 0:
       pTilde = -1
     hypotheticalPrior = volume.pow((pTilde-2)/2).detach()
     hypotheticalPrior = hypotheticalPrior / hypotheticalPrior.sum()
     axis[PRI].plot(grid, hypotheticalPrior, color="gray", linestyle='dotted')
