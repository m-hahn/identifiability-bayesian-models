...
P = sys.argv[1]
assert P in ["SECOND","BEST"] or int(P) > 0
...
FILE = f"logs/CROSSVALID/{__file__.replace('_VIZ_OnlyModel_OtherNoiseLevels_WithGroundTruthPrior', '')}_{FIT}_{P}_{FOLD_HERE}_{REG_WEIGHT}_{GRID}.txt"
...
print(FIT)
PRIOR, ENCODING = FIT.replace(".txt","").split("_")[-2:]
print(PRIOR, ENCODING)
import counterfactualComponents

prior_parameters = {}
counterfactualComponents.setPrior(PRIOR, prior_parameters, grid, MAX_GRID=360)
...
def model(grid):
  ...
  for iteration in range(1):
   ...
   prior_ground_truth = torch.nn.functional.softmax(prior_parameters["prior"], dim=0)

   loss = 0

   if iteration % 100 == 0:
     gridspec = dict(width_ratios=[1,0.5,1,0,0,0,0,0,0])
     figure, axis = plt.subplots(1, 9, figsize=(1.2*.4*6,.4*3), gridspec_kw=gridspec)
     figure.subplots_adjust(wspace=0.0, hspace=0.0, bottom=0.2)  # Increase bottom margin
     ...
     axis[PRI].plot(grid, prior_ground_truth.detach(), color="gray", linestyle="dotted")
...
#     for w in [PRI, ENC]:
 #      axis[w].tick_params(labelbottom=False)
   ...
   for w in range(9):
       axis[w].set_xticks(ticks=[0,180,360], labels=["0", "", "180"])

   if iteration % 100 == 0:
     plt.tight_layout()
     savePlot(f"figures/{__file__}_{FIT}_{PForStoring}_{FOLD_HERE}_{REG_WEIGHT}_{GRID}.pdf", bbox_inches='tight')
     print(f"figures/{__file__}_{FIT}_{PForStoring}_{FOLD_HERE}_{REG_WEIGHT}_{GRID}.pdf")
############################3
