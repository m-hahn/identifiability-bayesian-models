...
MAPCircularEstimator.set_parameters(GRID=GRID, OPTIMIZER_VERBOSE=OPTIMIZER_VERBOSE, KERNEL_WIDTH=KERNEL_WIDTH, MIN_GRID=MIN_GRID, MAX_GRID=MAX_GRID)
...
def samplePredictions(stimulus_, sigma_logit, prior, volumeElement, n_samples=100, showLikelihood=False, grid=grid, responses_=None, parameters=None, computePredictions=False, subject=None, sigma_stimulus=None, sigma2_stimulus=None, duration_=None, folds=None):
 ...
 if True:
  ...
  motor_noise = MakeFloatTensor(motor_noise)
...
parameters["sigma_logit"] = MakeFloatTensor(parameters["sigma_logit"].cpu().numpy().tolist() + additionalSigmaLogits)
...
assert len(ENCODING_List) == len(NoiseLevels)

counterfactualComponents.setPrior(PRIOR, parameters, grid, MAX_GRID)
overall_volume = MakeZeros(10, GRID)
for i, condition in enumerate(list([int(x) for x in NoiseLevels])):
  counterfactualComponents.setEncoding(ENCODING_List[i], parameters, grid, MAX_GRID)
  overall_volume[condition] = parameters["volume"]
parameters["volume"] = overall_volume
...
for condition in list([int(x) for x in NoiseLevels]):
   ...
   volume_base = torch.nn.functional.softmax(parameters["volume"][condition], dim=0)
   volume = 2 * math.pi * volume_base
