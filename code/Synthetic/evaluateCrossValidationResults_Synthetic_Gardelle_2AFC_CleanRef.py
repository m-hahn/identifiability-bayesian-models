...
with open(f"output/{__file__}_{FIT}.tex", "w") as outFile:
 for loss in range(0,15): # Ignore MAP for now
    if loss == 1:
        cosine_freeprior = crossValidResults(f"Run_2AFC_Synthetic_FreePrior_L1Loss_OnSim_CleanRef.py_{FIT}_{loss}_*_10.0_180.txt", STRICT=False)
    elif loss == 0:
       cosine_freeprior = crossValidResults(f"Run_2AFC_Synthetic_FreePrior_ZeroTrig_OnSim_CleanRef.py_{FIT}_{loss}_*_10.0_180.txt", STRICT=False)
    else:
       cosine_freeprior = crossValidResults(f"Run_2AFC_Synthetic_FreePrior_CosineLoss_OnSim_CleanRef"+("_WithStimNoise" if "Stim" in FIT else "")+f".py_{FIT}_{loss}_*_10.0_180.txt", STRICT=False)
    COLOR_FREE = "green"
    ...
    if RELEVANT_EXP == 1:
        reference = crossValidResults(f"Run_2AFC_Synthetic_FreePrior_L1Loss_OnSim_CleanRef.py_{FIT}_{RELEVANT_EXP}_*_10.0_180.txt", STRICT=False)
    elif RELEVANT_EXP == 0:
       reference = crossValidResults(f"Run_2AFC_Synthetic_FreePrior_ZeroTrig_OnSim_CleanRef.py_{FIT}_{RELEVANT_EXP}_*_10.0_180.txt", STRICT=False)
    else:
       reference = crossValidResults(f"Run_2AFC_Synthetic_FreePrior_CosineLoss_OnSim_CleanRef"+("_WithStimNoise" if "Stim" in FIT else "")+f".py_{FIT}_{RELEVANT_EXP}_*_10.0_180.txt", STRICT=False)
...
#    plotEffectOfLossFunction(COLOR_UNIFORM_PRIOR, "dotted", loss, cosine_uniprior, crossValidResults(f"Run_2AFC_Synthetic_FreePrior_CosineLoss_OnSim_CleanRef"+("_WithStimNoise" if "Stim" in FIT else "")+f"_F.py_{FIT}_{RELEVANT_EXP}_*_10.0_180.txt", STRICT=False))
#    plotEffectOfLossFunction(COLOR_UNIFORM_ENCODING, "dotted", loss, cosine_uniencoding, crossValidResults(f"Run_2AFC_Synthetic_FreePrior_CosineLoss_OnSim_CleanRef"+("_WithStimNoise" if "Stim" in FIT else "")+f"_F.py_{FIT}_{RELEVANT_EXP}_*_10.0_180.txt", STRICT=False))
