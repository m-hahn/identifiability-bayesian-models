...
NoiseLevelsSensory = sys.argv[7] # "5"
NoiseLevelsStimulus = sys.argv[8] #"25"
...
#     axis[ENC].set_yticks(ticks=[0,0.2,0.4], labels=["0", "0.2", "0.4"])
   ...
   for DURATION in range(1,10):
    if str(DURATION) not in NoiseLevelsSensory:
      continue
    for sigma_stimulus in range(10):
     if str(sigma_stimulus) not in NoiseLevelsStimulus:
       continue
#    for SUBJECT in [1]:
     ...
     if iteration % 500 == 0:
       ...
       kappa_stimulus = 1/(torch.exp(-init_parameters["sigma2_stimulus"][sigma_stimulus]) * math.pow(180 / math.pi,2))
       kappa = 1/float((4 * torch.sigmoid(init_parameters["sigma_logit"][DURATION])))
       axis[ENC].plot(grid, 2*computeResourcesWithStimulusNoise(volume.detach(), kappa, kappa_stimulus).detach())
   ...
   if iteration % 100 == 0:
     savePlot(f"figures/{__file__}_{NoiseLevelsSensory}_{NoiseLevelsStimulus}_{PRIOR}_{ENCODING}_{P}_{FOLD_HERE}_{REG_WEIGHT}_{GRID}.pdf")
