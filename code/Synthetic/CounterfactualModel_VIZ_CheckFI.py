...
GRID = int(sys.argv[4])
PRIOR = sys.argv[5] #"UNIFORM"
ENCODING = sys.argv[6] #"PERIODIC"
NoiseLevels = sys.argv[7] # "5"
SHOW_PLOT = True
DEVICE = 'cpu'
...
parameters = {}
with open(f"logs/CROSSVALID/RunGardelle_FreePrior_CosineLoss.py_8_0_10.0_180.txt", "r") as inFile:
    l = next(inFile)
    l = next(inFile)
    for l in inFile:
      print(l[:20])
...
def computeBias(stimulus_, sigma_logit, prior, volumeElement, n_samples=100, showLikelihood=False, grid=grid, responses_=None, parameters=None, computePredictions=False, subject=None, sigma_stimulus=None, sigma2_stimulus=None, duration_=None, folds=None, lossReduce='mean'):
 ...
 if True:
  ...
  biggerThan = (SQUARED_STIMULUS_DIFFERENCE(grid.unsqueeze(0) - grid.unsqueeze(1)) < 0).float() + 0.5 * (grid.unsqueeze(0) == grid.unsqueeze(1)).float()
 # print(biggerThan)
...
def model(grid):
  ...
  for iteration in range(1):
   ...
   if iteration % 100 == 0:
     gridspec = dict(width_ratios=[1,0.5,1,0.5,1,0.5,1,0,0,0,0])
     figure, axis = plt.subplots(1, 11, figsize=(0.7*1.3*0.8*5/6*10*1.3,0.8*2), gridspec_kw=gridspec,  constrained_layout=True )
#     plt.tight_layout()
 #    figure.subplots_adjust(wspace=0.15, hspace=0.0)
#     plt.tight_layout(rect=[0, 0.09, 1, 0.9])
     # rect = [left, bottom, right, top] in fraction of figure size
     
     # if you still need to adjust inter‐axis space:
#     figure.subplots_adjust(wspace=0.25)
     ...
     HUM = 8
     PAD = [1,3,5,7,8,9,10]
     axis[PRI].plot(grid.cpu(), prior.detach().cpu(), color="gray")
   ...
   for DURATION in range(2,6):
    for SUBJECT in [1]:
     ...
     loss_model, bayesianEstimate_model, bayesianEstimate_sd_byStimulus_model, attraction_model, fisherInformation,sqrtFisherInformation, sensory_variance = computeBias(xValues, init_parameters["sigma_logit"][DURATION], prior, volume, n_samples=1000, grid=grid, responses_=observations_y, parameters=parameters, computePredictions=(iteration%100 == 0), subject=None, sigma_stimulus=0, sigma2_stimulus=0, duration_=DURATION, folds=testFolds, lossReduce='sum')
     loss += loss_model

      
   #  uniform_prior = torch.nn.functional.softmax(MakeZeros(GRID), dim=0)
  #   _, repulsiveComponentOnly, _, _ = computeBias(xValues, init_parameters["sigma_logit"][DURATION], uniform_prior, volume, n_samples=1000, grid=grid, responses_=observations_y, parameters=parameters, computePredictions=(iteration%100 == 0), subject=None, sigma_stimulus=0, sigma2_stimulus=0, duration_=DURATION, folds=testFolds, lossReduce='sum')
     ...
     if iteration % 100 == 0:
       ...
       axis[ENC2].plot(grid, 2*fisherInformation.sqrt())
       axis[ENC3].plot(grid, 2*sqrtFisherInformation)
       axis[ENC4].plot(grid, 2*(1/sensory_variance.sqrt()))
     ...
     #if ENCODING == "UNIFORM":
     #    attraction_model = bayesianEstimate_model-grid

     axis[ATT].plot(grid, (attraction_model).detach()/2)

     # Two possible operationalizations of repulsion.
     axis[REP].plot(grid, (bayesianEstimate_model-attraction_model-grid).detach()/2)
#     axis[REP].plot(grid, (repulsiveComponentOnly-grid).detach()/2)
