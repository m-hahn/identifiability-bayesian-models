...
for condition in list([int(x) for x in NoiseLevels]):
   ...
   volume_base = torch.nn.functional.softmax(parameters["volume"], dim=0)
   volume_sec = volume_base + parameters["additive"][condition]
   volume_third = volume_sec / volume_sec.sum()
   volume = 2 * math.pi * volume_third
