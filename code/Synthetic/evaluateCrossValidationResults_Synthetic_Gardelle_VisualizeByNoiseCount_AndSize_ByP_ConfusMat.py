...
try:
  ...
  with open(f"output/{__file__}.txt", "w") as outFile:
    print("\t".join(["dataSize", "sensoryLevels", "noiseCount", "PFit", "NLL", "PTrue"]), file=outFile)
#       print("\t".join([str(q) for q in [dataSize.replace("N",""), sensoryLevels, len(sensoryLevels), q[0], q[2], groundTruthExp]]), file=outFile)
...
#    colorMap = {"0" : "black", "1": "red", "2": "blue", "3": "green", "4": "purple", "5": "orange", "6" : "black", "7" : "brown", "8" : "pink", "9" : "gray"}
    ...
    print(data)
    for q in data:
       print("\t".join([str(q) for q in [dataSize.replace("N",""), sensoryLevels, len(sensoryLevels), int(q[0]), q[2], groundTruthExp]]), file=outFile)
