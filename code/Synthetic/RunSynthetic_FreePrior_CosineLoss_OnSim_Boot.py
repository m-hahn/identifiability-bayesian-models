...
BOOT = random.randint(100,10000000)
...
########
# resample via bootstrapping
randState = random.Random(BOOT)
bootstrapIndices = [randState.randint(0, sample.size()[0]-1) for _ in range(sample.size()[0])]
observations_x = observations_x[bootstrapIndices]
observations_y = observations_y[bootstrapIndices]
sample = sample[bootstrapIndices]
responses = responses[bootstrapIndices]
#Subject = Subject[bootstrapIndices]
duration = duration[bootstrapIndices]
##############################################
  ...
  for iteration in range(10000000):
   ...
   if iteration % 500 == 0 and iteration > 0:
       ...
       if len(lossesBy500) > 0 and float(crossValidLoss) <= min(crossLossesBy500):
        with open(f"BOOT/losses/{__file__.replace('_VIZ', '')}_{FIT}_{P}_{FOLD_HERE}_{REG_WEIGHT}_{GRID}_{BOOT}.txt.txt", "w") as outFile:
            print(float(crossValidLoss), file=outFile)
        with open(f"BOOT/logs/CROSSVALID/{__file__}_{FIT}_{P}_{FOLD_HERE}_{REG_WEIGHT}_{GRID}_{BOOT}.txt", "w") as outFile:
