...
def model(grid):
  ...
  for iteration in range(1):
   ...
   if iteration % 100 == 0:
     ...
     figure, axis = plt.subplots(1, 9, figsize=(1.1*1.2*.4*6,.4*3), gridspec_kw=gridspec)
...
#     axis[ENC].set_yticks(ticks=[0,0.2,0.4], labels=["0", "0.2", "0.4"])
   ...
   for DURATION in range(2,6):
    ...
    for SUBJECT in [1]:
     ...
     if True:
       axis[PRI].set_title("Prior", fontsize=12)
       axis[ENC].set_title("Resources", fontsize=12)
       axis[ATT].set_title("Attraction", fontsize=12)
       axis[REP].set_title("Repulsion", fontsize=12)
       axis[TOT].set_title("Bias", fontsize=12)
       axis[HUM].set_title("Data", fontsize=12)


   for w in range(9):
     axis[w].tick_params(axis='x', labelsize=10)
     axis[w].tick_params(axis='y', labelsize=10)
   axis[ENC].text(430, -0.11, "[deg]", fontsize=9)
