...
ENCODING = sys.argv[6] #"STEEPPERIODIC"
NoiseLevels = sys.argv[7] #"25"
  ...
  for iteration in range(1):
   ...
   for DURATION in range(2,6):
    ...
    for SUBJECT in [1]:
     ...
     loss_model, bayesianEstimate_model, bayesianEstimate_sd_byStimulus_model, attraction_model = computeBias(xValues, init_parameters["sigma_logit"][DURATION], prior, volume, n_samples=1000, grid=grid, responses_=observations_y, parameters=parameters, computePredictions=(iteration%100 == 0), subject=None, sigma_stimulus=0, sigma2_stimulus=float(DURATION), duration_=DURATION, folds=testFolds, lossReduce='sum')
     loss += loss_model
