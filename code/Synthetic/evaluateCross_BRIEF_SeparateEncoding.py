...
with open(f"output/{__file__}_{FIT}.tex", "w") as outFile:
 for loss in [0,1,2,4,6,8]: # Ignore MAP for now
    if loss == 0:
       cosine_freeprior = crossValidResults(f"RunSynthetic_FreePrior_ZeroTrig_OnSim_SeparateEncoding.py_{FIT}_{loss}_*_10.0_180.txt", STRICT=False)
    elif loss == 1:
       cosine_freeprior = crossValidResults(f"RunSynthetic_FreePrior_L1Loss_OnSim_SeparateEncoding.py_{FIT}_{loss}_*_10.0_180.txt", STRICT=False)
    else:
       cosine_freeprior = crossValidResults(f"RunSynthetic_FreePrior_CosineLoss_OnSim"+("_WithStimNoise" if "Stim" in FIT else "")+f"_SeparateEncoding.py_{FIT}_{loss}_*_10.0_180.txt", STRICT=False)
    COLOR_FREE = "green"
    ...
    if GROUNDTRUTH_EXP == 0:
        reference = crossValidResults(f"RunSynthetic_FreePrior_ZeroTrig_OnSim_SeparateEncoding.py_{FIT}_{GROUNDTRUTH_EXP}_*_10.0_180.txt", STRICT=False)
    elif GROUNDTRUTH_EXP == 1:
        reference = crossValidResults(f"RunSynthetic_FreePrior_L1Loss_OnSim_SeparateEncoding.py_{FIT}_{GROUNDTRUTH_EXP}_*_10.0_180.txt", STRICT=False)
    else:
       reference = crossValidResults(f"RunSynthetic_FreePrior_CosineLoss_OnSim"+("_WithStimNoise" if "Stim" in FIT else "")+f"_SeparateEncoding.py_{FIT}_{GROUNDTRUTH_EXP}_*_10.0_180.txt", STRICT=False)
