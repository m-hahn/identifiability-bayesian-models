...
FILE = f"logs/CROSSVALID/{__file__.replace('_BootVIZ_OnlyModel_OtherNoiseLevels_Fig2', '')}_{FIT}_{P}_{FOLD_HERE}_{REG_WEIGHT}_{GRID}.txt"
...
# if (not (duration__ == duration).all()):
#    print("Waning: Durations are not matched to the human data.")
duration = duration__
...
loadModel(FILE, init_parameters, tolerant_for_sensory_noise_levels_mismatch=True)
...
#     axis[ENC].set_yticks(ticks=[0,0.2,0.4], labels=["0", "0.2", "0.4"])
   ...
   prior_estimate = prior.detach()
#   axis[PRI].plot(grid, prior.detach(), color="gray", alpha=0.02)
   ...
   for DURATION in range(1,10):
    ...
    for SUBJECT in [1]:
     ...
     if iteration % 100 == 0:
       #axis[ENC].plot(grid, 2*computeResources(volume.detach(), inverse_variance=1/float((4 * torch.sigmoid(init_parameters["sigma_logit"][DURATION])))), alpha=0.02)
       enc_estimate = 2*computeResources(volume.detach(), inverse_variance=1/float((4 * torch.sigmoid(init_parameters["sigma_logit"][DURATION])))).detach()

   prior_boot = []
   enc_boot = []
   print(f"BOOT/logs/CROSSVALID/{__file__.replace('VIZ_OnlyModel_OtherNoiseLevels_Fig2', '')}_{FIT}_{P}_{FOLD_HERE}_{REG_WEIGHT}_{GRID}_*.txt")
   for FILE in glob.glob(f"BOOT/logs/CROSSVALID/{__file__.replace('VIZ_OnlyModel_OtherNoiseLevels_Fig2', '')}_{FIT}_{P}_{FOLD_HERE}_{REG_WEIGHT}_{GRID}_*.txt"): #[:20]:
      print(FILE)
      loadModel(FILE, init_parameters, tolerant_for_sensory_noise_levels_mismatch=True)
      parameters = init_parameters
      # Define prior and resource allocation
      ...
      prior = torch.nn.functional.softmax(parameters["prior"], dim=0)
#      axis[PRI].plot(grid, prior.detach(), color="gray", alpha=0.02)
      prior_boot.append(prior.detach())
 #     axis[PRI].set_ylim(bottom=0)
      ...
      for DURATION in range(1,10):
       ...
       for SUBJECT in [1]:
        ...
        if iteration % 100 == 0:
#          axis[ENC].plot(grid, 2*computeResources(volume.detach(), inverse_variance=1/float((4 * torch.sigmoid(init_parameters["sigma_logit"][DURATION])))), alpha=0.02)
          enc_boot.append(2*computeResources(volume.detach(), inverse_variance=1/float((4 * torch.sigmoid(init_parameters["sigma_logit"][DURATION])))).detach())
   prior_boot = torch.stack(prior_boot, dim=1)
   enc_boot = torch.stack(enc_boot, dim=1)
   def SD(t):
       return (t.pow(2).mean(dim=1) - t.mean(dim=1).pow(2)).sqrt()
   print(prior_boot.size(), enc_boot.size())
   se_prior = SD(prior_boot)
   se_enc = SD(enc_boot)
#   print(se_prior)
   print(enc_boot.size(), enc_estimate.size(), se_enc.size())
   Zb_prior = ((prior_boot - prior_estimate.unsqueeze(1)) / (se_prior.unsqueeze(1) + 1e-8)).abs().max(dim=1).values
   c095_prior = torch.quantile(Zb_prior, 0.975)
   band_prior = se_prior #* c095_prior
   Zb_enc = ((enc_boot - enc_estimate.unsqueeze(1)) / (se_enc.unsqueeze(1) + 1e-8)).abs().max(dim=1).values
   c095_enc = torch.quantile(Zb_enc, 0.975)
   band_enc = se_enc #* c095_enc
   print(enc_estimate)
   print(band_enc)
 #  axis[ENC].fill_between(grid, enc_estimate-band_enc, enc_estimate+band_enc, color="gainsboro", edgecolor="none")
#   axis[ENC].fill_between(grid, torch.quantile(enc_boot, 0.025, dim=1), torch.quantile(enc_boot, 0.975, dim=1), color="gainsboro", edgecolor="none")
   axis[ENC].fill_between(grid, torch.quantile(enc_boot, 0.0, dim=1), torch.quantile(enc_boot, 1, dim=1), color="gainsboro", edgecolor="none")
  # axis[ENC].plot(grid, torch.quantile(enc_boot, 0.025, dim=1), color="dimgray")
  # axis[ENC].plot(grid, torch.quantile(enc_boot, 0.975, dim=1), color="dimgray")
   #print(torch.quantile(enc_boot, 0.025, dim=1))
   #print(torch.quantile(enc_boot, 0.975, dim=1))
   #quit()
   axis[ENC].plot(grid, enc_estimate, color="dimgray")
#   axis[PRI].fill_between(grid, torch.quantile(prior_boot, 0.025, dim=1), torch.quantile(prior_boot, 0.975, dim=1), color="gainsboro", edgecolor="none")
   axis[PRI].fill_between(grid, torch.quantile(prior_boot, 0.0, dim=1), torch.quantile(prior_boot, 1, dim=1), color="gainsboro", edgecolor="none")
 #  axis[PRI].fill_between(grid, prior_estimate-band_prior, prior_estimate+band_prior, color="gainsboro", edgecolor="none")
   axis[PRI].plot(grid, prior_estimate, color="dimgray")
#   quit()

   axis[PRI].set_ylim(bottom=0, top=1.1*(prior_estimate + band_prior).max())
   ...
   if iteration % 100 == 0:
     plt.tight_layout()
     savePlot(f"figures/{__file__}_{FIT}_{P}_{FOLD_HERE}_{REG_WEIGHT}_{GRID}.pdf", bbox_inches='tight')
