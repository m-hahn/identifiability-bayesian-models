...
parameters["additive"] = MakeFloatTensor([5,4,3,2,1,0])/200
...
for i, condition in enumerate(list([int(x) for x in NoiseLevels])):
  ...
  overall_volume[i] = parameters["volume"]
...
#     axis[ENC].set_yticks(ticks=[0,0.2,0.4], labels=["0", "0.2", "0.4"])
   ...
   for DUR_idx, DURATION in enumerate([int(x) for x in NoiseLevels]):
    
    volume_base = torch.nn.functional.softmax(parameters["volume"][DUR_idx], dim=0)
