...
NoiseLevelsSensory = sys.argv[8] #"25"
NoiseLevelsStimulus = sys.argv[9] #"25"
...
noiseLevelsByTrial = [int(NoiseLevelsSensory[i%len(NoiseLevelsSensory)]) for i in range(duration.size()[0])]
random.Random(11).shuffle(noiseLevelsByTrial)
duration = torch.LongTensor(noiseLevelsByTrial)



noiseLevelsByTrial = [int(NoiseLevelsStimulus[i%len(NoiseLevelsStimulus)]) for i in range(duration.size()[0])]
random.Random(12).shuffle(noiseLevelsByTrial)
StimulusSD = torch.LongTensor(noiseLevelsByTrial)
...
for condition in list([int(x) for x in NoiseLevelsSensory]):
 assert condition > 0 # should NOT be zero
 for stimNoise in list([int(x) for x in NoiseLevelsStimulus]):
   MASK = torch.logical_and(StimulusSD == stimNoise, duration == condition)
   observations_y[MASK] = samplePredictions(xValues[MASK], parameters["sigma_logit"][condition], prior, volume, n_samples=1000, grid=grid, responses_=observations_y, parameters=parameters, StimulusSD=StimulusSD, computePredictions=False, subject=None, sigma2_stimulus=(torch.exp(-parameters["sigma2_stimulus"][stimNoise]) if stimNoise > 0 else 0), Duration_=condition, folds=None, lossReduce='sum')
...
outPath = f"logs/SIMULATED_REPLICATE/{__file__}_{GRID}_{P}_{NoiseLevelsSensory}_{NoiseLevelsStimulus}_N{dataSize}_{PRIOR}_{ENCODING}.txt"
...
with open(outPath, "w") as outFile:
    ...
    for i in range(xValues.size()[0]):
      print(int(duration[i]), int(StimulusSD[i]), round(float(grid[xValues[i]]),3), round(float(observations_y[i]),3), file=outFile)
