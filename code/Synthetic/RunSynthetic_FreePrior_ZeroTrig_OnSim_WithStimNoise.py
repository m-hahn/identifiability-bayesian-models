...
duration__, StimulusSD__, sample__, responses__ = zip(*data)
StimulusSD__ = MakeLongTensor([int(q) for q in StimulusSD__])
StimulusSD = StimulusSD__
...
init_parameters = {}
init_parameters["sigma2_stimulus"] = MakeFloatTensor(10*[0]).view(10)
...
#    print("CONDITION", sigma_stimulus, duration_, folds)
...
def model(grid):
  ...
  for iteration in range(10000000):
   ...
   for DURATION in range(1,10):
    for sigma_stimulus in range(10):
     if torch.logical_and(StimulusSD == sigma_stimulus, duration == DURATION).long().sum() == 0:
       continue
     for SUBJECT in [1]:
      ## Run the model at its current parameter values.
      loss_model, bayesianEstimate_model, bayesianEstimate_sd_byStimulus_model, attraction_model = computeBias(xValues, init_parameters["sigma_logit"][DURATION], prior, volume, n_samples=1000, grid=grid, responses_=observations_y, parameters=parameters, computePredictions=(iteration%100 == 0), subject=None, sigma_stimulus=sigma_stimulus, sigma2_stimulus=torch.exp(-parameters["sigma2_stimulus"][sigma_stimulus]) if sigma_stimulus > 0 else 0, duration_=DURATION, folds=trainFolds, lossReduce='sum')
      loss += loss_model
   ...
   if iteration % 500 == 0:
     ...
     for DURATION in range(1,10):
      for sigma_stimulus in range(10):
       if torch.logical_and(StimulusSD == sigma_stimulus, duration == DURATION).long().sum() == 0:
        continue
       if torch.logical_and(torch.logical_and(StimulusSD == sigma_stimulus, duration==DURATION), (Fold == FOLD_HERE)).long().sum() == 0:
         print("Warning: test fold doesn't contain condition", (sigma_stimulus, DURATION, duration[Fold==FOLD_HERE]))
         continue

       for SUBJECT in [1]:
        loss_model, bayesianEstimate_model, bayesianEstimate_sd_byStimulus_model, attraction_model = computeBias(xValues, init_parameters["sigma_logit"][DURATION], prior, volume, n_samples=1000, grid=grid, responses_=observations_y, parameters=parameters, computePredictions=(iteration%500 == 0), subject=None, sigma_stimulus=sigma_stimulus, sigma2_stimulus=torch.exp(-parameters["sigma2_stimulus"][sigma_stimulus]) if sigma_stimulus > 0 else 0, duration_=DURATION, folds=testFolds, lossReduce='sum')
