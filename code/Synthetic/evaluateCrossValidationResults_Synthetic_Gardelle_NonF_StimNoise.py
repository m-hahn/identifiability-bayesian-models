...
# losses/RunSynthetic_FreePrior_CosineLoss_OnSim_WithStimNoise.py_SimulateSynthetic_Parameterized_OtherNoiseLevels_Grid_WithStimNoise.py_1800_4_25_0_STEEPPERIODIC_UNIFORM.txt_8_0_10.0_180.txt.txt
# RunSynthetic_FreePrior_ZeroTrig_OnSim_WithStimNoise.py

with open(f"output/{__file__}_{FIT}.tex", "w") as outFile:
 for loss in [0,1,2,4,6,8]: # Ignore MAP for now
    if loss == 0:
       cosine_freeprior = crossValidResults(f"RunSynthetic_FreePrior_ZeroTrig_OnSim_WithStimNoise.py_{FIT}_{loss}_*_10.0_180.txt", STRICT=False)
    elif loss == 1:
       cosine_freeprior = crossValidResults(f"RunSynthetic_FreePrior_L1Loss_OnSim_WithStimNoise.py_{FIT}_{loss}_*_10.0_180.txt", STRICT=False)
    else:
       cosine_freeprior = crossValidResults(f"RunSynthetic_FreePrior_CosineLoss_OnSim_WithStimNoise.py_{FIT}_{loss}_*_10.0_180.txt", STRICT=False)
    COLOR_FREE = "green"
    ...
    if GROUNDTRUTH_EXP == 0:
        reference = crossValidResults(f"RunSynthetic_FreePrior_ZeroTrig_OnSim_WithStimNoise.py_{FIT}_{GROUNDTRUTH_EXP}_*_10.0_180.txt", STRICT=False)
    elif GROUNDTRUTH_EXP == 1:
        reference = crossValidResults(f"RunSynthetic_FreePrior_L1Loss_OnSim_WithStimNoise.py_{FIT}_{GROUNDTRUTH_EXP}_*_10.0_180.txt", STRICT=False)
    else:
       reference = crossValidResults(f"RunSynthetic_FreePrior_CosineLoss_OnSim_WithStimNoise.py_{FIT}_{GROUNDTRUTH_EXP}_*_10.0_180.txt", STRICT=False)
