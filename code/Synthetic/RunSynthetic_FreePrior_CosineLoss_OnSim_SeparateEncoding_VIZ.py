...
assert "Additive" in FIT or "Separate" in FIT
...
init_parameters["volume"] = MakeZeros(10,GRID)
...
#     axis[ENC].set_yticks(ticks=[0,0.2,0.4], labels=["0", "0.2", "0.4"])
   ...
   for DURATION in range(1,10):
    if (duration == DURATION).long().sum() == 0:
       continue
    volume_base = torch.nn.functional.softmax(parameters["volume"][DURATION], dim=0)
